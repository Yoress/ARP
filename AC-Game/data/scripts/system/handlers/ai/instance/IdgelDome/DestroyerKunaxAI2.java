...
public class DestroyerKunaxAI2 extends AggressiveNpcAI2 {
	...
	protected void handleTargetGiveup() {
		...
		// The Destroyer Kunax will stop focusing its target and return to its
		// original position.
	...
	private boolean chooseBossSkill() {
		...
		// if (getOwner().isCasting() || isAlreadyDead() || isBossOutsideCircle
		// || getOwner().getTarget() == null || isInSubState(AISubState.CAST) ||
		// attackCounter > 20) {
		// return false;
		// }
	...
	private void CleavingMassacre() {
		// SKILL = CleavingMassacre (21553) =
		// IDLDF5_Fortress_Re_WingBlade_TA_Strong
	...
	private void AerialConfinement() {
		// SKILL = AerialConfinement (21555) =
		// IDLDF5_Fortress_Re_AreaOpenAerial_forACT
...
//	}
					...
					if (!isAlreadyDead() && !getOwner().isCasting()) {
						...
						if (distance >= 23.01) {
							...
							if (player == null) {
								...
								// The Destroyer Kunax will stop focusing its
								// target and return to its original position.
								sendMsgByRace(1402578, Race.PC_ALL, 0);
								
								// Point randPoint =
								// MathUtil.get2DPointOnCircle(new Point((int)
								// 264.4382f, (int)258.58527f), new Point((int)
								// getOwner().getX(), (int)getOwner().getY()),
								// 16);
	...
	public AttackIntention chooseAttackIntention(Creature creature) {
		...
		if (currentTarget == null || !currentTarget.getObjectId().equals(mostHated.getObjectId())) {
			onCreatureEvent(AIEventType.TARGET_CHANGED, mostHated); //FIXME: Called by think() from SWITCH_TARGET intention
		 ...
		 * 
		 * The Z-axis difference between Boss and Player is "ToleranceZ" 0 = Ground Attacks 1 = Air Attacks (v4.7.5.17)
		 */
