...
public class PortalDialogAI2 extends PortalAI2 {
	...
	public boolean onDialogSelect(Player player, int dialogId, int questId, int extendedRewardIndex) {
		...
		if (dialogId == DialogAction.INSTANCE_PARTY_MATCH.id()) {
			 ...
			 * else if (dialogId == DialogAction.SELECT_ACTION_1012.id()) { PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(getObjectId(),
			 * 1182)); }
	...
	private void checkDialog(Player player) {
		...
		if (playerHasQuest) {
			...
			case 804625:
				PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(getObjectId(), 4762, 0)); // instances
																									 // with
																									 // infernal
																									 // dialog.
				break;
