...
//					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(env.getVisibleObject().getObjectId(), 10));
//					return true;
					return defaultCloseDialog(env, var, var, true, 0, 0, 182208013, 1);
				}
			...
			case USE_OBJECT:
				if (var == 5) {
					if (!giveQuestItem(env, 182208015, 1)) { // Second Rubbed
															 // Copy
						return false;
					}
			...
			case USE_OBJECT:
				if (var == 6) {
					if (!giveQuestItem(env, 182208016, 1)) { // Third Rubbed
															 // Copy
						return false;
					}
