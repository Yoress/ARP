...
public class _2213PoisonRootPotentFruit extends QuestHandler {
			...
			case 203604: {
				if (qs.getQuestVarById(0) == 1) {
					if (env.getDialog() == DialogAction.QUEST_SELECT) {
						...
						removeQuestItem(env, 182203208, 1); // Remove Okaru
															 // Fruit item [ID:
															 // 182203208]
						player.getEffectController().removeEffect(1851); // Remove
																		 // Okaru
																		 // Log
																		 // Poison
																		 // effect
	...
	public boolean onGetItemEvent(QuestEnv env) {
		final Player player = env.getPlayer();
		SkillEngine.getInstance().applyEffectDirectly(1851, player, player, 0); // Add
																				 // Okaru
																				 // Log
																				 // Poison
																				 // effect
