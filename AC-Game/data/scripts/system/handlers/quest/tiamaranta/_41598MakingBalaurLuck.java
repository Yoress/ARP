...
import static com.aionemu.gameserver.network.aion.serverpackets.SM_SYSTEM_MESSAGE.STR_QUEST_GET_REWARD_ERROR_NO_QUEST_ITEM_MULTIPLE;
			...
			if (targetId == startingNpcId) {
				...
				case QUEST_ACCEPT_SIMPLE:
					if (!player.getInventory().isFullSpecialCube()) {
						...
						return sendQuestSelectionDialog(env); //TODO: remove this nonsense when special cube size doesn't matter for quests.
					}
				case QUEST_REFUSE_SIMPLE:
				case FINISH_DIALOG: // To avoid double opening on quest selected window if closed
					return closeDialogWindow(env);
				default:
					return sendQuestSelectionDialog(env); //FIXME: This is wrong? have to look at the quest to determine what to do.
				}
			}
