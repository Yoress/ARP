...
public class MovePlayerTo extends AdminCommand {
	...
	public void execute(Player player, String... params) {
		...
		if (params[1].contains("0")) {
			...
			goTo(player, player1, worldid, x, y, z);
		...
		if (destination.equalsIgnoreCase("Sanctum")) {
			goTo(player, player1, WorldMapType.SANCTUM.getId(), 1322, 1511, 568);
		} // Kaisinel
		else if (destination.equalsIgnoreCase("Kaisinel")) {
			goTo(player, player1, WorldMapType.KAISINEL.getId(), 2155, 1567, 1205);
		} // Poeta
		else if (destination.equalsIgnoreCase("Poeta")) {
			goTo(player, player1, WorldMapType.POETA.getId(), 806, 1242, 119);
		} else if (destination.equalsIgnoreCase("Melponeh")) {
			goTo(player, player1, WorldMapType.POETA.getId(), 426, 1740, 119);
		} // Verteron
		else if (destination.equalsIgnoreCase("Verteron")) {
			goTo(player, player1, WorldMapType.VERTERON.getId(), 1643, 1500, 119);
		} else if (destination.equalsIgnoreCase("Cantas") || destination.equalsIgnoreCase("Cantas Coast")) {
			goTo(player, player1, WorldMapType.VERTERON.getId(), 2384, 788, 102);
		} else if (destination.equalsIgnoreCase("Ardus") || destination.equalsIgnoreCase("Ardus Shrine")) {
			goTo(player, player1, WorldMapType.VERTERON.getId(), 2333, 1817, 193);
		} else if (destination.equalsIgnoreCase("Pilgrims") || destination.equalsIgnoreCase("Pilgrims Respite")) {
			goTo(player, player1, WorldMapType.VERTERON.getId(), 2063, 2412, 274);
		} else if (destination.equalsIgnoreCase("Tolbas") || destination.equalsIgnoreCase("Tolbas Village")) {
			goTo(player, player1, WorldMapType.VERTERON.getId(), 1291, 2206, 142);
		} // Eltnen
		else if (destination.equalsIgnoreCase("Eltnen")) {
			goTo(player, player1, WorldMapType.ELTNEN.getId(), 343, 2724, 264);
		} else if (destination.equalsIgnoreCase("Golden") || destination.equalsIgnoreCase("Golden Bough Garrison")) {
			goTo(player, player1, WorldMapType.ELTNEN.getId(), 688, 431, 332);
		} else if (destination.equalsIgnoreCase("Eltnen Observatory")) {
			goTo(player, player1, WorldMapType.ELTNEN.getId(), 1779, 883, 422);
		} else if (destination.equalsIgnoreCase("Novan")) {
			goTo(player, player1, WorldMapType.ELTNEN.getId(), 947, 2215, 252);
		} else if (destination.equalsIgnoreCase("Agairon")) {
			goTo(player, player1, WorldMapType.ELTNEN.getId(), 1921, 2045, 361);
		} else if (destination.equalsIgnoreCase("Kuriullu")) {
			goTo(player, player1, WorldMapType.ELTNEN.getId(), 2411, 2724, 361);
		} // Theobomos
		else if (destination.equalsIgnoreCase("Theobomos")) {
			goTo(player, player1, WorldMapType.THEOBOMOS.getId(), 1398, 1557, 31);
		} else if (destination.equalsIgnoreCase("Jamanok") || destination.equalsIgnoreCase("Jamanok Inn")) {
			goTo(player, player1, WorldMapType.THEOBOMOS.getId(), 458, 1257, 127);
		} else if (destination.equalsIgnoreCase("Meniherk")) {
			goTo(player, player1, WorldMapType.THEOBOMOS.getId(), 1396, 1560, 31);
		} else if (destination.equalsIgnoreCase("obsvillage")) {
			goTo(player, player1, WorldMapType.THEOBOMOS.getId(), 2234, 2284, 50);
		} else if (destination.equalsIgnoreCase("Josnack")) {
			goTo(player, player1, WorldMapType.THEOBOMOS.getId(), 901, 2774, 62);
		} else if (destination.equalsIgnoreCase("Anangke")) {
			goTo(player, player1, WorldMapType.THEOBOMOS.getId(), 2681, 847, 138);
		} // Heiron
		else if (destination.equalsIgnoreCase("Heiron")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 2540, 343, 411);
		} else if (destination.equalsIgnoreCase("Heiron Observatory")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 1423, 1334, 175);
		} else if (destination.equalsIgnoreCase("Senea")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 971, 686, 135);
		} else if (destination.equalsIgnoreCase("Jeiaparan")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 1635, 2693, 115);
		} else if (destination.equalsIgnoreCase("Changarnerk")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 916, 2256, 157);
		} else if (destination.equalsIgnoreCase("Kishar")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 1999, 1391, 118);
		} else if (destination.equalsIgnoreCase("Arbolu")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 170, 1662, 120);
		} else if (destination.equalsIgnoreCase("reaper")) {
			goTo(player, player1, WorldMapType.HEIRON.getId(), 2767, 1867, 154);
		} /**
			 * Asmodae
			 */
		// Pandaemonium
		else if (destination.equalsIgnoreCase("Pandaemonium")) {
			goTo(player, player1, WorldMapType.PANDAEMONIUM.getId(), 1679, 1400, 195);
		} // Marchutran
		else if (destination.equalsIgnoreCase("Marchutan")) {
			goTo(player, player1, WorldMapType.MARCHUTAN.getId(), 1557, 1429, 266);
		} // Ishalgen
		else if (destination.equalsIgnoreCase("Ishalgen")) {
			goTo(player, player1, WorldMapType.ISHALGEN.getId(), 529, 2449, 281);
		} else if (destination.equalsIgnoreCase("Anturon")) {
			goTo(player, player1, WorldMapType.ISHALGEN.getId(), 940, 1707, 259);
		} // Altgard
		else if (destination.equalsIgnoreCase("Altgard")) {
			goTo(player, player1, WorldMapType.ALTGARD.getId(), 1748, 1807, 254);
		} else if (destination.equalsIgnoreCase("Basfelt")) {
			goTo(player, player1, WorldMapType.ALTGARD.getId(), 1903, 696, 260);
		} else if (destination.equalsIgnoreCase("Trader")) {
			goTo(player, player1, WorldMapType.ALTGARD.getId(), 2680, 1024, 311);
		} else if (destination.equalsIgnoreCase("Impetusium")) {
			goTo(player, player1, WorldMapType.ALTGARD.getId(), 2643, 1658, 324);
		} else if (destination.equalsIgnoreCase("Altgard Observatory")) {
			goTo(player, player1, WorldMapType.ALTGARD.getId(), 1468, 2560, 299);
		} // Morheim
		else if (destination.equalsIgnoreCase("Morheim")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 308, 2274, 449);
		} else if (destination.equalsIgnoreCase("Desert")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 634, 900, 360);
		} else if (destination.equalsIgnoreCase("Slag")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 1772, 1662, 197);
		} else if (destination.equalsIgnoreCase("Kellan")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 1070, 2486, 239);
		} else if (destination.equalsIgnoreCase("Alsig")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 2387, 1742, 102);
		} else if (destination.equalsIgnoreCase("Morheim Observatory")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 2794, 1122, 171);
		} else if (destination.equalsIgnoreCase("Halabana")) {
			goTo(player, player1, WorldMapType.MORHEIM.getId(), 2346, 2219, 127);
		} // Brusthonin
		else if (destination.equalsIgnoreCase("Brusthonin")) {
			goTo(player, player1, WorldMapType.BRUSTHONIN.getId(), 2917, 2421, 15);
		} else if (destination.equalsIgnoreCase("Baltasar")) {
			goTo(player, player1, WorldMapType.BRUSTHONIN.getId(), 1413, 2013, 51);
		} else if (destination.equalsIgnoreCase("Bollu")) {
			goTo(player, player1, WorldMapType.BRUSTHONIN.getId(), 840, 2016, 307);
		} else if (destination.equalsIgnoreCase("Edge")) {
			goTo(player, player1, WorldMapType.BRUSTHONIN.getId(), 1523, 374, 231);
		} else if (destination.equalsIgnoreCase("Bubu")) {
			goTo(player, player1, WorldMapType.BRUSTHONIN.getId(), 526, 848, 76);
		} else if (destination.equalsIgnoreCase("Settlers")) {
			goTo(player, player1, WorldMapType.BRUSTHONIN.getId(), 2917, 2417, 15);
		} // Beluslan
		else if (destination.equalsIgnoreCase("Beluslan")) {
			goTo(player, player1, WorldMapType.BELUSLAN.getId(), 398, 400, 222);
		} else if (destination.equalsIgnoreCase("Besfer")) {
			goTo(player, player1, WorldMapType.BELUSLAN.getId(), 533, 1866, 262);
		} else if (destination.equalsIgnoreCase("Kidorun")) {
			goTo(player, player1, WorldMapType.BELUSLAN.getId(), 1243, 819, 260);
		} else if (destination.equalsIgnoreCase("Red Mane")) {
			goTo(player, player1, WorldMapType.BELUSLAN.getId(), 2358, 1241, 470);
		} else if (destination.equalsIgnoreCase("Kistenian")) {
			goTo(player, player1, WorldMapType.BELUSLAN.getId(), 1942, 513, 412);
		} else if (destination.equalsIgnoreCase("Hoarfrost")) {
			goTo(player, player1, WorldMapType.BELUSLAN.getId(), 2431, 2063, 579);
		} /**
			 * Balaurea
			 */
		// Inggison
		else if (destination.equalsIgnoreCase("Inggison")) {
			goTo(player, player1, WorldMapType.INGGISON.getId(), 1335, 276, 590);
		} else if (destination.equalsIgnoreCase("Ufob")) {
			goTo(player, player1, WorldMapType.INGGISON.getId(), 382, 951, 460);
		} else if (destination.equalsIgnoreCase("Soteria")) {
			goTo(player, player1, WorldMapType.INGGISON.getId(), 2713, 1477, 382);
		} else if (destination.equalsIgnoreCase("Hanarkand")) {
			goTo(player, player1, WorldMapType.INGGISON.getId(), 1892, 1748, 327);
		} // Gelkmaros
		else if (destination.equalsIgnoreCase("Gelkmaros")) {
			goTo(player, player1, WorldMapType.GELKMAROS.getId(), 1763, 2911, 554);
		} else if (destination.equalsIgnoreCase("Subterranea")) {
			goTo(player, player1, WorldMapType.GELKMAROS.getId(), 2503, 2147, 464);
		} else if (destination.equalsIgnoreCase("Rhonnam")) {
			goTo(player, player1, WorldMapType.GELKMAROS.getId(), 845, 1737, 354);
		} // Silentera
		else if (destination.equalsIgnoreCase("Silentera")) {
			goTo(player, player1, 600010000, 583, 767, 300);
		} /**
			 * Abyss
			 */
		else if (destination.equalsIgnoreCase("Reshanta")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 951, 936, 1667);
		} else if (destination.equalsIgnoreCase("Teminon")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 2867, 1034, 1528);
		} else if (destination.equalsIgnoreCase("Primum")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 1078, 2839, 1636);
		} else if (destination.equalsIgnoreCase("Tigraki")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 539, 1100, 2843);
		} else if (destination.equalsIgnoreCase("Nuage noir") || destination.equalsIgnoreCase("Nuage")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 3429, 2439, 2765);
		} else if (destination.equalsIgnoreCase("Leibos")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 2136, 1943, 1597);
		} else if (destination.equalsIgnoreCase("ORSL") || destination.equalsIgnoreCase("Ori sup Latesran")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 1596, 2952, 2943);
		} else if (destination.equalsIgnoreCase("OCSL") || destination.equalsIgnoreCase("Occi sup Latesran")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 2054, 660, 2843);
		} else if (destination.equalsIgnoreCase("ORIL") || destination.equalsIgnoreCase("Ori inf Latesran")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 1639, 2968, 1668);
		} else if (destination.equalsIgnoreCase("OCIL") || destination.equalsIgnoreCase("Occi inf Latesran")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 2095, 679, 1567);
		} else if (destination.equalsIgnoreCase("Eye of Reshanta") || destination.equalsIgnoreCase("Eye")) {
			goTo(player, player1, WorldMapType.RESHANTA.getId(), 1979, 2114, 2291);
	...
	private static void goTo(final Player admin, final Player player, int worldId, float x, float y, float z) {
...
//			TeleportService2.teleportTo(player, worldId, getInstanceId(worldId, player), x, y, z);
			summonPlayerTo(admin, player, 30, worldId, getInstanceId(worldId, player), x, y, z, player.getHeading());
		} else {
			int instanceId = 1;
...
//			TeleportService2.teleportTo(player, worldId, x, y, z);
			summonPlayerTo(admin, player, 30, worldId, instanceId, x, y, z, player.getHeading());
		}
	}
	
	/**
	 * Prompts the summonTarget to accept a summon to the location given within timeout seconds.
	 * 
	 ...
	 * @param worldId -- the worldId of the destination
	 * @param instanceId -- the instanceId of the destination
	 ...
	 */
	private static void summonPlayerTo(Player admin, Player summonTarget, int timeout, int worldId, int instanceId, float x, float y, float z, byte h) {
