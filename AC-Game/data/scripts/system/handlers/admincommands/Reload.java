		...
		if (params[0].equals("quest")) {
			...
			try {
				...
				for (File file : listFiles(dir, true)) {
					...
					if (data != null) if (data.getQuest() != null) questScriptsData.getQuest().addAll(data.getQuest());
				}
			...
			try {
				...
				for (File file : listFiles(dir, true)) {
					...
					if (data != null) newTemplates.addAll(data.getSkillTemplates());
				}
			...
			if (data != null) {
				...
				if (text == null || text.trim().length() == 0) text = "NONE";
