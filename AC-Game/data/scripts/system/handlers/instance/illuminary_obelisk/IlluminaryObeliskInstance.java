...
public class IlluminaryObeliskInstance extends GeneralInstanceHandler {
	...
	/**
	 * Reward: After a successful capture of the boss you will get a small chance of obtaining mythical wings, and a variety of items. Boxes are for
	...
	public void onDropRegistered(Npc npc) {
		...
		case 233740: // Test Weapon Dynatoum.
			for (Player player : instance.getPlayersInside()) {
				if (player.isOnline()) {
					dropItems.add(DropRegistrationService.getInstance().regDropItem(index++, player.getObjectId(), npcId, 188053083, 1)); // Tempering
																																			 // Solution
																																			 // Chest.
				}
				switch (Rnd.get(1, 2)) {
				case 1:
					dropItems.add(DropRegistrationService.getInstance().regDropItem(1, 0, npcId, 188052830, 1)); // Dynatoum's
																												 // Brazen
																												 // Weapon
																												 // Box.
					break;
				case 2:
					dropItems.add(DropRegistrationService.getInstance().regDropItem(1, 0, npcId, 188052831, 1)); // Dynatoum's
																												 // Brazen
																												 // Armor
																												 // Box.
					break;
			...
			break;
		/**
		 * Collect Items: Each "Shield Generator" unit needs 3 ide items, 12 items in total, you can find them all around the instance. Bombs to use
		 * the cannons appear in chests around the instance in a different place every time, collect them too.
		 */
	...
	/**
	 * Defense Cannons: Each Shield Unit has a defense cannon that can be used. This cannons do powerful wide area damage attacks. In order to use
	 * them you need to have Bomb items. When a shield is charged completely a cannon will spawn to help in the defense of the area. Determining a
	 * person to use the cannon and positioning before the mobs come is a recommended. Bombs to use the cannons appear in chests around the instance
	 * in a different place every time, collect them too.
	 */
	@Override
	...
	public void onDie(Npc npc) {
		...
		case 233740: // Test Weapon Dynatoum.
			...
			spawn(702018, 258.888f, 251.14882f, 455.12198f, (byte) 105); // Supply
																		 // Box.
			spawn(730905, 255.5597f, 254.49713f, 455.12012f, (byte) 104); // Illuminary
																			 // Obelisk
																			 // Exit.
			break;
		}
	...
	public void onEnterZone(Player player, ZoneInstance zone) {
		if (zone.getAreaTemplate().getZoneName() == ZoneName.get("SHIELD_GENERATION_HUB_301230000")) {
			...
			if (zone.getAreaTemplate().getZoneName() == ZoneName.get("SZ_SECTOR_A_301230000") || zone.getAreaTemplate().getZoneName() == ZoneName.get("SZ_SECTOR_B_301230000")
					|| zone.getAreaTemplate().getZoneName() == ZoneName.get("SZ_SECTOR_C_301230000") || zone.getAreaTemplate().getZoneName() == ZoneName.get("SZ_SECTOR_D_301230000")) {
				if (!isStop1) {
					chargecheck();
	...
	public void onLeaveInstance(Player player) {
		...
		removeEffects(player);
		stopInstanceTask();
		PacketSendUtility.sendPacket(player, new SM_QUEST_ACTION(0, 0)); // cancel
																		 // timer
	}
