...
public class DragonLordsRefuge extends GeneralInstanceHandler {
	...
	public void onDie(Npc npc) {
		...
		case 219368: // petriscale
			...
			performSkillToTarget(219361, 219361, 20982); // remove Petrification
														 // Buff
			sendMsg(1401536);
			checkIncarnationKills();
			break;
		...
		case 219359: // Calindi Flamelord
			...
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					spawn(219361, 466.7468f, 514.5500f, 417.4044f, (byte) 0);// tiamat
																				// dragon
																				// 2nd
																				// Spawn
					...
					performSkillToTarget(219361, 219361, 20978); // Petrification
																 // Buff
					performSkillToTarget(219361, 219361, 20984); // Unbreakable
																 // Wing
																 // (reflect)
				}
			}, 5000);
			
			// schedule dragon lords roar skill to block all players before
			// spawn empyrean lords
			ThreadPoolManager.getInstance().schedule(new Runnable() {
		...
		case 219362: // Tiamat Dragon (3rd spawn)
			...
			spawn(701542, 480f, 514f, 417.405f, (byte) 0);// tiamat treasure
															// chest reward
			...
			spawn(800464, 544.964f, 517.898f, 417.405f, (byte) 113); // reian
																	 // sorcerer
			spawn(800465, 545.605f, 510.325f, 417.405f, (byte) 17); // reian
																	 // sorcerer
			break;
		case 283163: // balaur spiritualist (spawn Portal after die)
			healEmpyreanLord(0); // heal Empyrean Lord
			spawn(730675, 460.082f, 571.978f, 417.405f, (byte) 43); // spawn
																	 // portal to
																	 // tiamat
																	 // incarnation
			break;
		...
		case 283166:
			...
			break;
		case 219361: // Tiamat Dragon (1st spawn) - Players cannot kill tiamat,
					 // they must kill 4 incanation before
			break;
		...
		case 219491: // Marchutan Gods (1st Spawn)
			...
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					spawn(730694, 436.7526f, 513.8103f, 420.6662f, (byte) 0, 14); // re-spawn
																					 // tiamat
																					 // aetheric
																					 // field
					spawn(219360, 451.9700f, 514.5500f, 417.4044f, (byte) 0); // re-spawn
																				 // tiamat
																				 // woman
																				 // to
																				 // initial
					...
					spawn(730630, 548.18683f, 514.54523f, 420f, (byte) 0, 23); // spawn
																				 // exit
				}
			}, 5000);
			break;
	...
	private void healEmpyreanLord(int id) {
		...
		if (npc != null && !NpcActions.isAlreadyDead(npc)) {
			SkillEngine.getInstance().getSkill(npc, skill, 60, npc).useNoAnimationSkill(); // heal
																							 // 7%
																							 // +
																							 // def
																							 // buff
	...
	private void checkIncarnationKills() {
		...
		if (killedCount == 4) {
			...
			npc.getEffectController().removeEffect(20984);// dispel Unbreakable
															// Wing (reflect)
			sendMsg(1401537);
			// schedule spawn of empyrean lords for final attack to tiamat
			// before became exausted
			ThreadPoolManager.getInstance().schedule(new Runnable() {
	...
	@Override
	public void onDropRegistered(Npc npc) { // Dragon Lord Tiamat Chest for all
											 // members
