	...
	protected void reward() {
		...
		for (Player player : IdgelDome.getPlayersInside()) {
			...
			if (player.getRace().equals(idgelDomeReward.getWinningRace())) {
				...
				playerReward.setReward1Count(6f * PlayerRateModifire);// Winner
																		// Team
																		// always
																		// got 6
																		// <Fragmented
																		// Ceramium>
				playerReward.setReward2Count(1f * PlayerRateModifire);// Winner
																		// Team
																		// always
																		// got 1
																		// <Idgel
				...
				playerReward.setReward1Count(2f * PlayerRateModifire);// Looser
																		// Team
																		// always
																		// got 2
																		// <Fragmented
																		// Ceramium>
				playerReward.setReward2Count(0f * PlayerRateModifire);// Winner
																		// Team
																		// always
																		// got 0
																		// <Idgel
			...
			ItemService.addItem(player, 186000243, (long) playerReward.getReward1Count());// Fragmented
																							// Ceramium
			ItemService.addItem(player, 188053030, (long) playerReward.getReward2Count());// Idgel
																							// Dome
																							// Reward
																							// Box
...
//	}
	...
	protected void stopInstance(Race race) {
		...
		reward();
		/* Reward Packet */
		IdgelDomePackets.sendScoreTypePacket();// Send 1-Time when
												// Start_Progress and
												// End_Progress
		IdgelDomePackets.sendRewardPacket();// Send Reward Packet
											// OnBossKilled()|OnTimeOut()
	}
