...
public class FFAInstance extends GeneralInstanceHandler {
	...
	static Point3D[] positions = new Point3D[] {new Point3D(EventSystem.FFA_SPAWNPOINT_1X, EventSystem.FFA_SPAWNPOINT_1Y, EventSystem.FFA_SPAWNPOINT_1Z),
	...
	private void checkIfSameMac(Player winner, Player loser) {
		...
		if ((mac1 != null) && (mac2 != null)) {
			if ((ip1.equalsIgnoreCase(ip2)) && (mac1.equalsIgnoreCase(mac2))) {
				AuditLogger.info(winner, "[TradeKillAlert] You really need to check player " + winner.getName() + " and " + loser.getName()
						+ ", They have same IP and MAC and possible they are Trade Killing in FFA, so please go and check in invisible! (MAC: " + mac1 + ").");
				int lose_ap = 30000;
				int lose_gp = 5000;
				int omegaId = 166020000;
				int tsId = 166030005;
				AbyssPointsService.addAp(winner, -lose_ap); // reducing ap from
															 // trade killers
				AbyssPointsService.addAp(loser, -lose_ap);
				
				AbyssPointsService.addGp(winner, -lose_gp); // reducing gp from
															 // trade killers
				AbyssPointsService.addGp(loser, -lose_gp);
				
				winner.getInventory().decreaseByItemId(omegaId, 1); // removing
																	 // Omega
																	 // from
																	 // trade
																	 // killers
				...
				loser.getInventory().decreaseByItemId(tsId, 1); // removeing TS
																 // from trade
																 // killers
				winner.getInventory().decreaseByItemId(tsId, 1);
				PacketSendUtility.sendMessage(winner, "[TradeKillAlert] You lost " + lose_ap + " AP for Trade Killing!"); // AP
																															 // Lose
																															 // msg
				PacketSendUtility.sendMessage(loser, "[TradeKillAlert] You lost " + lose_ap + " AP for Trade Killing!");
				PacketSendUtility.sendMessage(winner, "[TradeKillAlert] You lost " + lose_gp + " GP for Trade Killing!"); // GP
																															 // Lose
																															 // msg
				PacketSendUtility.sendMessage(loser, "[TradeKillAlert] You lost " + lose_gp + " GP for Trade Killing!");
				PacketSendUtility.sendMessage(winner, "[TradeKillAlert] You lost [item:" + omegaId + "] x 1 and [item:" + tsId + "] x 1 item(s) for Trade Killing!"); // item
																																										 // lose
																																										 // Item
																																										 // msg
				PacketSendUtility.sendMessage(loser, "[TradeKillAlert] You lost [item:" + omegaId + "] x 1 and [item:" + tsId + "] x 1 item(s) for Trade Killing!"); // item
																																									 // lose
																																									 // Item
																																									 // msg
	...
	public void KillingSpreeMsg(final String msg) {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player object) {
				if (object.getWorldId() == instance.getMapId() && object.isInFFA()) PacketSendUtility.sendSys2Message(object, EventSystem.FFA_ANNOUNCER_NAME, msg);
			}
		});
	}
