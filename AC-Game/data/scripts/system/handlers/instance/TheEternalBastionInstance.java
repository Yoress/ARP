...
public class TheEternalBastionInstance extends GeneralInstanceHandler {
	...
	public void onDie(Npc npc) {
		...
		case 231170: // Commander.
			...
			if (Wave01Begin == 3) {
				...
				// Uderground
				spawn(231171, 656.05f, 212.84f, 223.95f, (byte) 74); // Commander
																	 // Matuk
				// Left Flang
				spawn(231172, 607.57f, 405.07f, 224.09f, (byte) 92); // Commander
																	 // Badut
				spawn(233313, 609.18f, 407.78f, 224.12f, (byte) 92); // Guard
				spawn(233313, 605.69f, 407.75f, 224.01f, (byte) 92); // Guard
				// Right FLang
				spawn(231173, 652.82f, 461.09f, 225.62f, (byte) 98); // Commander
																	 // Kastu
		...
		case 231173: // Commander.
			...
			if (Wave02Begin == 3) {
				...
				// Top
				spawn(231176, 758.98f, 393.70f, 243.35f, (byte) 47); // Commander
																	 // Nitra
				// Left Flang
				spawn(231175, 584.28f, 371.35f, 225.35f, (byte) 113); // Commander
																		 // Kaimdu
				...
				// Right FLang
				spawn(231174, 652.82f, 461.09f, 225.62f, (byte) 98); // Commander
																	 // Murat
				spawn(233313, 659.12f, 457.89f, 225.76f, (byte) 98); // Guard
				spawn(233313, 650.74f, 454.19f, 225.86f, (byte) 98); // Guard
				spawn(233315, 652.53f, 470.91f, 225.70f, (byte) 98); // Guard
		...
		case 231176: // Commander.
			...
			if (Wave03Begin == 3) {
				...
				spawn(231143, 613.27f, 262.20f, 228.26f, (byte) 3); // Pashid
																	 // Siege
																	 // Tower.
				spawn(231152, 608.41f, 303.55f, 228.29f, (byte) 113); // Pashid
																		 // Siege
																		 // Tower.
				spawn(231153, 626.25f, 351.28f, 226.01f, (byte) 113); // Pashid
																		 // Siege
																		 // Tower.
				spawn(231154, 672.08f, 416.16f, 228.01f, (byte) 75); // Pashid
																	 // Siege
																	 // Tower.
				spawn(231155, 633.40f, 395.50f, 226.62f, (byte) 13); // Pashid
																	 // Siege
																	 // Tower.
			}
			despawnNpc(npc);
		...
		case 231155: // Pashid Siege Tower.
			...
			if (Wave04Begin == 5) { // 5 Wave Begin
				...
				spawn(231130, 744.22f, 293.07f, 233.69f, (byte) 42); // General
																	 // Fly
																	 // Fashid.
				spawn(231131, 712.75f, 289.28f, 249.28f, (byte) 1); // Guard Fly
																	 // Fashid.
				spawn(231131, 747.78f, 323.02f, 249.28f, (byte) 88); // Guard
																	 // Fly
																	 // Fashid.
			}
			despawnNpc(npc);
		...
		case 231130: // Boss Rank S.
			...
			if (checkRank(instanceReward.getPoints()) == 1) {
				spawn(701913, 744.22f, 293.07f, 233.69f, (byte) 42); // Chest
																	 // Rank
																	 // S.
			} else if (checkRank(instanceReward.getPoints()) == 2) {
				spawn(701914, 744.22f, 293.07f, 233.69f, (byte) 42); // Chest
																	 // Rank
																	 // A.
			} else if (checkRank(instanceReward.getPoints()) == 3) {
				spawn(701915, 744.22f, 293.07f, 233.69f, (byte) 42); // Chest
																	 // Rank
																	 // B.
			} else if (checkRank(instanceReward.getPoints()) == 4) {
				spawn(701916, 744.22f, 293.07f, 233.69f, (byte) 42); // Chest
																	 // Rank
																	 // C.
			} else if (checkRank(instanceReward.getPoints()) == 5) {
				spawn(701917, 744.22f, 293.07f, 233.69f, (byte) 42); // Chest
																	 // Rank
																	 // D.
			}
			final int bastion_exit = spawnRace == Race.ASMODIANS ? 730872 : 730871;
			spawn(bastion_exit, 767.10693f, 264.60303f, 233.49748f, (byte) 43); // The
																				 // Eternal
																				 // Bastion
																				 // Exit.
			break;
		...
		case 209517: // Kill Commander Asmodians
			...
			spawn(bastion_exit2, 767.10693f, 264.60303f, 233.49748f, (byte) 43); // The
																				 // Eternal
																				 // Bastion
																				 // Exit.
	...
	public void handleUseItemFinish(Player player, Npc npc) {
		...
		case 701625: // Eternal Bastion Turet Elyos.
			spawn(231160, 707.61f, 260.72f, 253.43f, (byte) 38); // Pashid
																 // Assault
																 // Pod.
			spawn(230744, 703.81f, 264.90f, 253.43f, (byte) 43); // Pashid
																 // Assault
																 // officer.
			spawn(230744, 705.67f, 270.07f, 253.43f, (byte) 43); // Pashid
																 // Assault
																 // officer.
			spawn(230745, 704.81f, 262.90f, 253.43f, (byte) 43); // Pashid
																 // Assault
																 // Commander.
			despawnNpc(npc);
			break;
	...
	private void startAssaultPodTimer() {
		...
		WaveAssaultFirst = ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				spawn(231140, 745.09f, 302.52f, 233.75f, (byte) 88); // Pashid
																	 // Assault
																	 // Pod.
				spawn(231141, 735.23f, 293.94f, 233.75f, (byte) 118); // Pashid
																		 // Assault
																		 // Drill.
			}
		}, 5000);
		...
		AssaultPodAndDrill = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
				...
				switch (rnd) {
				case 1:
					spawn(231156, 771.80f, 358.20f, 230.96f, (byte) 27); // Pashid
																		 // Assault
																		 // Pod.
					spawn(231163, 755.40f, 363.19f, 230.96f, (byte) 43); // Pashid
																		 // Assault
																		 // Drill.
					TimerDespawn(98000);
					break;
				case 2:
					spawn(231157, 729.01f, 358.72f, 230.94f, (byte) 31); // Pashid
																		 // Assault
																		 // Pod.
					spawn(231164, 735.94f, 372.56f, 230.94f, (byte) 73); // Pashid
																		 // Assault
																		 // Drill.
					TimerDespawn(98000);
					break;
				case 3:
					spawn(231158, 677.17f, 313.80f, 225.69f, (byte) 56); // Pashid
																		 // Assault
																		 // Pod.
					spawn(231165, 673.63f, 302.47f, 225.69f, (byte) 57); // Pashid
																		 // Assault
																		 // Drill.
					TimerDespawn(98000);
					break;
				case 4:
					spawn(231159, 659.70f, 257.36f, 225.69f, (byte) 3); // Pashid
																		 // Assault
																		 // Pod.
					spawn(231166, 653.85f, 266.61f, 225.69f, (byte) 118); // Pashid
																			 // Assault
																			 // Drill.
					TimerDespawn(98000);
					break;
				case 5:
					spawn(231160, 764.36f, 392.54f, 243.35f, (byte) 22); // Pashid
																		 // Assault
																		 // Pod.
					spawn(231167, 630.05f, 305.34f, 238.07f, (byte) 22); // Pashid
																		 // Assault
																		 // Drill.
					TimerDespawn(98000);
					break;
