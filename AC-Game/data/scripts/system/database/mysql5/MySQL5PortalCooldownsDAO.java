...
/**
 * @modified Yon (Aion Reconstruction Project) -- updated handling to use {@link PortalCooldown} objects.
 */
public class MySQL5PortalCooldownsDAO extends PortalCooldownsDAO {
	...
	public void loadPortalCooldowns(final Player player) {
		...
		try {
			...
			while (rset.next()) {
				...
				PortalCooldown cd = new PortalCooldown(worldId, reuseTime, entryCount);
				if (cd.shouldPersist(player, worldId)) {
					portalCoolDowns.put(worldId, cd);
				}
			}
	...
	public void storePortalCooldowns(final Player player) {
		...
		for (Map.Entry<Integer, PortalCooldown> entry : portalCoolDowns.entrySet()) {
			...
			if (!entry.getValue().shouldPersist(player, worldId)) {
				continue;
			}
