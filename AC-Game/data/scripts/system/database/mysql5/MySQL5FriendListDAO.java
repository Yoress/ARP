...
/**
 * @author Ben
 * @modified Yon (Aion Reconstruction Project) -- Added memo functionality.
 */
public class MySQL5FriendListDAO extends FriendListDAO {
	...
	public static final String ADD_QUERY = "INSERT INTO `friends` (`player`,`friend`,`memo`) VALUES (?, ?, ?)";
	public static final String UPDATE_MEMO_QUERY = "UPDATE `friends` SET `memo` = ? WHERE `player` = ? AND `friend` = ?";
	...
	public FriendList load(final Player player) {
		...
		try {
			...
			while (rset.next()) {
				...
				if (pcd != null) {
					Friend friend = new Friend(pcd, memo);
					friends.add(friend);
				}
			}
		} catch (Exception e) {
	...
	@Override
	public boolean updateMemo(final Player player, final Friend friend, final String memo) {
		return DB.insertUpdate(UPDATE_MEMO_QUERY, new IUStH() {
			
			@Override
			public void handleInsertUpdate(PreparedStatement ps) throws SQLException {
				ps.setString(1, memo);
				ps.setInt(2, player.getObjectId());
				ps.setInt(3, friend.getOid());
				ps.execute();
			}
		});
	}
