 ...
 * @reworked Yon (Aion Reconstruction Project) -- Removed GM GP rank tracking, and added a method to properly update top_ranking
 */
public class MySQL5AbyssRankDAO extends AbyssRankDAO {
	...
	public static final String SELECT_PLAYERS_RANKING = "SELECT abyss_rank.rank, abyss_rank.ap, abyss_rank.gp, abyss_rank.old_rank_pos, abyss_rank.rank_pos, players.name, legions.name, players.id, players.title_id, players.player_class, players.gender, players.exp FROM abyss_rank INNER JOIN players INNER JOIN "
			+ LOGIN_DATABASE + ".account_data ON abyss_rank.player_id = players.id AND " + LOGIN_DATABASE + ".account_data.id = players.account_id AND " + LOGIN_DATABASE
			+ ".account_data.access_level = 0 LEFT JOIN legion_members ON legion_members.player_id = players.id LEFT JOIN legions ON legions.id = legion_members.legion_id WHERE players.race = ? AND abyss_rank.gp > 0 ORDER BY abyss_rank.gp DESC LIMIT 0, 300";
	public static final String SELECT_PLAYERS_RANKING_ACTIVE_ONLY = "SELECT abyss_rank.rank, abyss_rank.ap, abyss_rank.gp, abyss_rank.old_rank_pos, abyss_rank.rank_pos, players.name, legions.name, players.id, players.title_id, players.player_class, players.gender, players.exp FROM abyss_rank INNER JOIN players INNER JOIN "
			+ LOGIN_DATABASE + ".account_data ON abyss_rank.player_id = players.id AND " + LOGIN_DATABASE + ".account_data.id = players.account_id AND " + LOGIN_DATABASE
			+ ".account_data.access_level = 0 LEFT JOIN legion_members ON legion_members.player_id = players.id LEFT JOIN legions ON legions.id = legion_members.legion_id WHERE players.race = ? AND abyss_rank.gp > 0 AND UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60 ORDER BY abyss_rank.gp DESC LIMIT 0, 300";
	...
	// public static final String SELECT_GP_PLAYER = "SELECT player_id, gp FROM abyss_rank, players WHERE abyss_rank.player_id = players.id AND
	// players.race = ? AND gp > ? ORDER by gp DESC";
	public static final String SELECT_GP_PLAYER = "SELECT abyss_rank.player_id, abyss_rank.gp FROM abyss_rank, players, " + LOGIN_DATABASE
			+ ".account_data WHERE abyss_rank.player_id = players.id AND players.account_id = " + LOGIN_DATABASE + ".account_data.id AND " + LOGIN_DATABASE
			+ ".account_data.access_level = 0 AND players.race = ? AND gp >= ? ORDER by gp DESC";
	// public static final String SELECT_GP_PLAYER_ACTIVE_ONLY = "SELECT player_id, gp FROM abyss_rank, players WHERE abyss_rank.player_id =
	// players.id AND players.race = ? AND gp > ? AND UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60 ORDER BY gp
	// DESC";
	public static final String SELECT_GP_PLAYER_ACTIVE_ONLY = "SELECT abyss_rank.player_id, abyss_rank.gp FROM abyss_rank, players, " + LOGIN_DATABASE
			+ ".account_data WHERE abyss_rank.player_id = players.id AND players.account_id = " + LOGIN_DATABASE + ".account_data.id AND " + LOGIN_DATABASE
			+ ".account_data.access_level = 0 AND players.race = ? AND gp >= ? AND UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60 ORDER by gp DESC";
	public static final String UPDATE_RANK = "UPDATE abyss_rank SET rank = ? WHERE player_id = ?";
	public static final String UPDATE_TOP_RANKING = "UPDATE abyss_rank SET top_ranking = ? WHERE player_id = ?";
	public static final String SELECT_LEGION_COUNT = "SELECT COUNT(player_id) as players FROM legion_members WHERE legion_id = ?";
	// public static final String UPDATE_PLAYER_RANK_LIST = "UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos
	// = @a:=@a+1 where player_id in (SELECT id FROM players where race = ?) order by gp desc" + (RankingConfig.TOP_RANKING_SMALL_CACHE ? " limit 500"
	// : ""); //only 300 positions are relevant later, so we update them + some extra positions that can get into the toprankings
	public static final String UPDATE_PLAYER_RANK_LIST = "UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT players.id FROM players, "
			+ LOGIN_DATABASE + ".account_data where players.race = ? AND " + LOGIN_DATABASE + ".account_data.access_level = 0 AND " + LOGIN_DATABASE
			+ ".account_data.id = players.account_id) order by gp desc" + (RankingConfig.TOP_RANKING_SMALL_CACHE ? " limit 500" : "");
	// public static final String UPDATE_PLAYER_RANK_LIST_ACTIVE_ONLY = "UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos,
	// abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT id FROM players where race = ? AND
	// UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60) order by gp desc" + (RankingConfig.TOP_RANKING_SMALL_CACHE ?
	// " limit 500" : ""); //only 300 positions are relevant later, so we update them + some extra positions that can get into the toprankings
	public static final String UPDATE_PLAYER_RANK_LIST_ACTIVE_ONLY = "UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT players.id FROM players, "
			+ LOGIN_DATABASE + ".account_data where players.race = ? AND " + LOGIN_DATABASE + ".account_data.access_level = 0 AND " + LOGIN_DATABASE
			+ ".account_data.id = players.account_id AND UNIX_TIMESTAMP(CURDATE())-UNIX_TIMESTAMP(players.last_online) <= ? * 24 * 60 * 60) order by gp desc"
			+ (RankingConfig.TOP_RANKING_SMALL_CACHE ? " limit 500" : "");
	public static final String UPDATE_LEGION_RANK_LIST = "UPDATE legions SET legions.old_rank_pos = legions.rank_pos, legions.rank_pos = @a:=@a+1 where id in (SELECT legion_id FROM legion_members, players where rank = 'BRIGADE_GENERAL' AND players.id = legion_members.player_id and players.race = ?) order by legions.contribution_points DESC"
			+ (RankingConfig.TOP_RANKING_SMALL_CACHE ? " limit 75" : ""); // only 50 positions are relevant later, so we update them + some extra
																			 // positions that can get into the toprankings
	...
	@Override
	public void updateTopRanking(int playerId, int topRanking) {
		Connection con = null;
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement(UPDATE_TOP_RANKING);
