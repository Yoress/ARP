		...
		DB.select("SELECT * FROM inventory WHERE `item_location` = 126", new ReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					brokerItems.add(new Item(itemUniqueId, itemId, itemCount, itemColor, colorExpireTime, itemCreator, expireTime, activationCount, false, false, slot, location, enchant, maxEnchant,
							itemSkin, fusionedItem, optionalSocket, optionalFusionSocket, charge, randomBonus, rndCount, packCount, max_authorize, false, isAmplified == 1, buffSkill));
				}
			}
		});
