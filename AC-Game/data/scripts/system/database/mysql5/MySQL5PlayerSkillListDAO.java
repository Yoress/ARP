 ...
 * @modified Yon (Aion Reconstruction Project) -- Added the exceptions to the error logging messages in {@link #store(Player, List)},
 * {@link #addSkills(Connection, Player, List)}, {@link #deleteSkills(Connection, Player, List)},
 * and {@link #updateSkills(Connection, Player, List)}; Changed {@link #skillsToDeletePredicate} to also accept NEW skills as a precaution.
 */
public class MySQL5PlayerSkillListDAO extends PlayerSkillListDAO {
	...
	private static final Predicate<PlayerSkillEntry> skillsToDeletePredicate = new Predicate<PlayerSkillEntry>() {
		...
		@Override
		public boolean apply(@Nullable PlayerSkillEntry input) {
			//Accepting NEW skills will prevent any skills that are being inserted from already existing in the database (it happens sometimes)
			return input != null && (PersistentState.DELETED == input.getPersistentState() || PersistentState.NEW == input.getPersistentState());
		}
	};
	...
	private void store(Player player, List<PlayerSkillEntry> skills) {
		...
		try {
			...
			log.error("Failed to open connection to database while saving SkillList for player " + player.getObjectId() + "; " + e.getMessage(), e);
		} finally {
			DatabaseFactory.close(con);
		}
	...
	private void addSkills(Connection con, Player player, List<PlayerSkillEntry> skills) {
		...
		try {
			...
			log.error("Can't add skills for player: " + player.getObjectId() + "; " + e.getMessage(), e);
		} finally {
	...
	private void updateSkills(Connection con, Player player, List<PlayerSkillEntry> skills) {
		...
		try {
			...
			log.error("Can't update skills for player: " + player.getObjectId() + "; " + e.getMessage(), e);
		} finally {
			DatabaseFactory.close(ps);
		}
	}
	...
	private void deleteSkills(Connection con, Player player, List<PlayerSkillEntry> skills) {
		...
		try {
			...
			log.error("Can't delete skills for player: " + player.getObjectId() + "; " + e.getMessage(), e);
		} finally {
			DatabaseFactory.close(ps);
		}
	}
