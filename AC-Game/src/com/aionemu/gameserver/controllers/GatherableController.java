...
public class GatherableController extends VisibleObjectController<Gatherable> {
	...
	public void onStartUse(final Player player) {
		...
		switch (result) {
		case 1: // player has equipped item, or have a consumable in inventory,
				 // so he will obtain extra items
	...
	public Material getMaterial() {
		...
		/*
		 * if (mats != null) { Material m = mats.select(); if (Rnd.chance(m.getRate())) { return m; } } return null;
		 */
	}
	...
	private boolean checkGatherable(final Player player, final GatherableTemplate template) {
		if (player.isNotGatherable()) {
			// You are currently poisoned and unable to extract. (Time
			// remaining: %DURATIONTIME0)
