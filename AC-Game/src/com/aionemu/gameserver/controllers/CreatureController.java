 ...
 * @modified Yon (Aion Reconstruction Project) -- Added a method to check for empty corpses and despawn them,
 * modified {@link #attackTarget(Creature, int)} to update Creature heading,
 * modified {@link #onAttack(Creature, int, TYPE, int, boolean, LOG)} and {@link #onDie(Creature)} to add
 * special handling for Mechanics System, {@link #onAttack(Creature, int, TYPE, int, boolean, LOG)} changed
 * to {@link #onAttack(Creature, int, TYPE, int, boolean, LOG, AttackStatus)} + fixed all resultant compile errors
 * for the Mechanics System, altered {@link #onDie(Creature)} and {@link #onDieSilence()} to no longer
 * set {@link CreatureState#FLOATING_CORPSE} for Npc's unless it was flying.
 */
public abstract class CreatureController<T extends Creature> extends VisibleObjectController<Creature> {
	...
	public void onDie(final Creature lastAttacker) {
		...
		getOwner().getKnownList().doOnAllNpcs(new Visitor<Npc>() { //TODO: Move into death observer
			@Override
			public void visit(Npc object) {
				if (object.getAi2() instanceof AbstractMechanicsAI2) {
					((AbstractMechanicsAI2) object.getAi2()).onSeeCreatureDie(getOwner(), lastAttacker.getActingCreature());
				}
			}
		});
...
//				if (((Npc) getOwner()).getObjectTemplate().isFloatCorpse()) {
		...
		if (getOwner() instanceof Npc) lootlessCorpseCheck();
	}
...
//				if (((Npc) getOwner()).getObjectTemplate().isFloatCorpse()) {
	...
	/**
	 * This method checks with {@link DropRegistrationService} to see if the {@link #getOwner() owner}
	 * has any loot on its corpse. If not, then the corpse is despawned.
	 * 
	 * If the owner is a Player, this method simply returns.
	 */
	private void lootlessCorpseCheck() {
		if (this.getOwner() instanceof Player) return;
		Set<DropItem> dropItems = DropRegistrationService.getInstance().getCurrentDropMap().get(this.getOwner().getObjectId());
		if (dropItems == null || dropItems.isEmpty()) {
			this.getOwner().getController().onDelete();
		}
	}
	 ...
	 * @param type
	 */
	public void onAttack(final Creature attacker, final int skillId, TYPE type, int damage, boolean notifyAttack, LOG log, AttackStatus status) {
		...
		// Do NOT notify attacked observers if the damage is 0 and shield is up
		// (means the attack has been absorbed)
		...
		if (getOwner() instanceof Npc) {
			...
			if (ai instanceof AbstractMechanicsAI2) {
				...
				AbstractMechanicsAI2 mAI = (AbstractMechanicsAI2) ai;
				if (damage > 0) mAI.onMechanicEvent(new GeneralMechanicEvent(MechanicEventType.on_damaged));
				if (skillId == 0) {
					mAI.onMechanicEvent(new AttackedEvent(MechanicEventType.on_attacked, getOwner(), null, attacker, status));
				} else {
					mAI.onMechanicEvent(new SkillEvent(MechanicEventType.on_spelled, skillId, getOwner(), attacker));
				}
			}
		...
		getOwner().getKnownList().doOnAllNpcs(new Visitor<Npc>() {
			...
			public void visit(Npc object) {
				...
				if (ai instanceof AbstractMechanicsAI2) {
					//Mechanic AI's are different -- they need the skill id to identify proper behaviour.
					((AbstractMechanicsAI2) ai).onSeeCreatureAttacked(getOwner(), skillId, attacker);
				} else {
					ai.onCreatureEvent(AIEventType.CREATURE_NEEDS_SUPPORT, getOwner());
				}
			}
		});
	}
	 ...
	 */
	public final void onAttack(Creature creature, int skillId, final int damage, boolean notifyAttack, AttackStatus status) {
		this.onAttack(creature, skillId, TYPE.REGULAR, damage, notifyAttack, LOG.REGULAR, status);
	}
	
	public final void onAttack(Creature creature, final int damage, boolean notifyAttack, AttackStatus status) {
		this.onAttack(creature, 0, TYPE.REGULAR, damage, notifyAttack, LOG.REGULAR, status);
	}
	...
	public void attackTarget(final Creature target, int time) {
		...
		//Update heading to face target -- no need to send packet, client will update visual heading on its own.
		//TODO: Test and check if it's needed in useSkill, too.
		if (getOwner() != target) {
			getOwner().getPosition().setH(MathUtil.estimateHeadingFrom(getOwner(), target));
		}
		...
		for (AttackResult result : attackResult) {
			if (status == null) status = result.getAttackStatus();
		...
		if (time == 0) {
			target.getController().onAttack(getOwner(), damage, true, status);
		} else {
			ThreadPoolManager.getInstance().schedule(new DelayedOnAttack(target, creature, damage, status), time);
		}
	}
	...
	public void cancelAllTasks() {
		for (int i : tasks.keySet()) {
			...
			if (task != null && i != TaskId.RESPAWN.ordinal()) { //TODO: Why are we not canceling respawn!?
				task.cancel(false);
			}
		}
		tasks.clear();
	...
	private static final class DelayedOnAttack implements Runnable {
		...
		public DelayedOnAttack(Creature target, Creature creature, int finalDamage, AttackStatus status) {
			this.target = target;
		...
		@Override
		public void run() {
			target.getController().onAttack(creature, finalDamage, true, status);
			target = null;
			creature = null;
		}
	}
