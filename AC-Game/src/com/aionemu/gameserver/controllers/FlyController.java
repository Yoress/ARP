 ...
 * @author ATracer
 * @rework Blackfire
 * @modified Yon (Aion Reconstruction Project) -- {@link #onStopGliding(boolean)} now checks if the player is in a flight zone before
 * returning them to normal flying... forcibly ending their flight if they are not.
 */
public class FlyController {
	
	@SuppressWarnings("unused")
	private static final Logger log = LoggerFactory.getLogger(FlyController.class); //TODO: Remove unusde logger.
	private static final long FLY_REUSE_TIME = 10000; //TODO: Consider making this a config?
	...
	/**
	 * Switching to glide mode (called by CM_MOVE with VALIDATE_GLIDE movement type) 1) from standing state 2) from flying state If from stand to
	 * glide - start fp reduce + emotions/stats if from fly to glide - only emotions/stats
	 */
