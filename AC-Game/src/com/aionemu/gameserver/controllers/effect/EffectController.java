 ...
 * @modified Yon (Aion Reconstruction Project) -- Pykachu submitted fix: {@link #checkEffectCooldownId(Effect)} modified to return for
 * flame cage and erosion, as they should stack; modified {@link #setAbnormal(int)} and {@link #unsetAbnormal(int)} to support
 * the Mechanics System, {@link #isAbnormalState(AbnormalState)} tweaked to return true if any of the abnormal states of the compounded
 * state are set; refactored {@link #setAbnormal(int)} to {@link #setAbnormal(int, Creature)} for Mechanics System support.
 */
public class EffectController {
	...
	/**
	 * Method used to calculate number of effects of given dispelcategory, targetslot and dispelLevel used only in DispelBuffCounterAtk, therefore
	 ...
	 * ABNORMAL EFFECTS
	 */
	public void setAbnormal(int mask, Creature caster) {
		AbnormalState state = AbnormalState.getStateById(mask);
		owner.getObserveController().notifyAbnormalSettedObservers(state);
		abnormals |= mask;
		
		if (owner instanceof Npc && owner.getAi2() instanceof AbstractMechanicsAI2) {
			AbstractMechanicsAI2 mAI = (AbstractMechanicsAI2) owner.getAi2();
			mAI.onMechanicEvent(new AbnormalStateEvent(MechanicEventType.on_enter_abnormal_state, state, caster));
		}
	}
	...
	public void unsetAbnormal(int mask) {
		...
		if (owner instanceof Npc && owner.getAi2() instanceof AbstractMechanicsAI2) {
			AbstractMechanicsAI2 mAI = (AbstractMechanicsAI2) owner.getAi2();
			mAI.onMechanicEvent(new AbnormalStateEvent(MechanicEventType.on_leave_abnormal_state, AbnormalState.getStateById(mask), null));
		}
	}
	...
	public boolean isAbnormalState(AbnormalState id) {
		...
		//<= is a bad check; Creature can still be under one of the compounded states and fail that <= check.
	...
	private boolean priorityStigmaEffect(Effect nextEffect) {
		for (Effect effect : abnormalEffectMap.values()) {
			if (effect.getSkillTemplate().getStigmaType().getId() < nextEffect.getSkillTemplate().getStigmaType().getId() && effect.getTargetSlot() == nextEffect.getTargetSlot()
