...
/**
 * @author ATracer
 * @modified Yon (Aion Reconstruction Project) -- Added some exceptions to being below the water level in {@link #checkZoneLevels(Player)}.
 */
public class ZoneLevelService {
	...
	public static void checkZoneLevels(Player player) {
		...
		//TODO: Some zones, such as this one in Altgard, are under the water level but are not meant to drown the player.
		boolean safeUnderwaterZone = false;
		if (player.isInsideZone(ZoneName.get("ALTGARD_FORTRESS_DUNGEON_220030000"))) safeUnderwaterZone = true;
		if (player.isInsideZone(ZoneName.get("ALTGARD_FORTRESS_DUNGEON_ENTRANCE_220030000"))) safeUnderwaterZone = true;
		
		
		// TODO need fix character height
		float playerheight = player.getPlayerAppearance().getHeight() * 1.6f;
		if (z < world.getWorldMap(player.getWorldId()).getWaterLevel() - playerheight && !safeUnderwaterZone) {
