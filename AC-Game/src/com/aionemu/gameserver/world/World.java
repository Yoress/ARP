 ...
 * @modified Yon (Aion Reconstruction Project) -- Added a try/catch for NullPointers in {@link #removeObject(VisibleObject) removeObject()} for bases,
 * fixed a race condition in {@link #despawn(VisibleObject, boolean)}.
 */
public class World {
	...
	public void removeObject(VisibleObject object) {
		...
		if (object.getSpawn() instanceof BaseSpawnTemplate) {
			...
			try {
				...
				/*
				 * This shit been throwing NullPointers even after I added checks for it. Do nothing, just let the thread continue.
				 */
			}
		}
	...
	/**
	 * Update position of VisibleObject [used when object is moving on one map instance]. Check if active map region changed and do all needed
	 * updates.
	 *
	 * @param object
	 * @param newX
	...
	/**
	 * Despawn VisibleObject, object will become invisible and object position will become invalid. All others objects will be noticed that this
	...
	public void despawn(VisibleObject object, boolean clearKnownlist) {
		...
		if (oldMapRegion != null) { // can be null if an instance gets deleted?
			WorldMapInstance oldMapRegionParent = oldMapRegion.getParent();
			if (oldMapRegionParent != null) {
				oldMapRegionParent.removeObject(object);
			}
