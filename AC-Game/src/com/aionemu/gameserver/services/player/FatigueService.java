...
public class FatigueService {
	...
	public void removeRecoverCount(Player player, int count) {
		...
		if (player.getCommonData().getFatigueRecover() < count) return;
		...
		for (final Player player : players) {
			delays.add(ThreadPoolManager.getInstance().scheduleAtFixedRate(new Runnable() {
				...
				public void run() {
					if (player == null) return;
					if (player.getLevel() < 10) return;
					if (player.getWorldId() == 110010000 || player.getWorldId() == 120010000) {// SANCTUM,
																								// PANDAEMONIUM
						return;
					}
					...
					if (player.getCommonData().getFatigue() > 100) player.getCommonData().setFatigue(100);
					checkFatigue(player);
	...
	public String fatigueMessage(int count) {
		if (count == 0) return "Your fatigue is empty";
	...
	public void onPlayerLogin(Player player) {
		...
		case Calendar.SUNDAY:
			if (player.getCommonData().getFatigueReset() == 1) player.getCommonData().setFatigueReset(0);
			break;
		}
