...
/**
 * @author ATracer
 * @modified Yon (Aion Reconstruction Project) -- Removed duplicate logout handling in {@link #startLeaveWorld(Player)},
 * added update to Account equipment item list so players who log to character select are wearing the correct gear.
 */
public class PlayerLeaveWorldService {
	 ...
	 * <br>
	 * <b><font color='red'>NOTICE: </font> This method is called only from {@link GameConnection} and {@link CM_QUIT} and must not be called from
	 * anywhere else</b>
	 */
	public static final void startLeaveWorld(Player player) {
		...
		//Both of these are called below
...
//		PlayerAllianceService.onPlayerLogout(player);
		...
		//update account equipment so characters show the correct armour on the selection screen when the player doesn't log out completely
		player.getPlayerAccount().getPlayerAccountData(player.getObjectId()).setEquipment(player.getEquipment().getPADEquipment());
