 ...
 * @modified Yon (Aion Reconstruction Project) -- removed in-game spam of dev credits, adjusted some hardcoded values to use server configs.
 */
...
//	private static final String serverIntro = "Please remember: Accountsharing is not permitted";
	...
	/**
	 * <tt>GameServer Presentation Settings</tt> This are only the main server information settings placeholders. You can customise your server if you
	 * wish, by editing the "CustomMessageId.java" file, located in the i18n folder You can also setup an individual language file, located in languages
	 * folder
	 * <p/>
	 * <p/>
	 * Please do not change here something, ragarding the developer credits, except the "developed by XXXX". Even if you edit a lot of files in this
	 * source, you still have no rights to call it as "your Core". Everybody knows that this Emulator Core was developed by Aion Lightning
	 */
	static {
...
//		alBuffer = alBuffer + "Copyright 2010-2015\n";
//		alBuffer = alBuffer + "=============================\n";
...
//		alBuffer = null;
	...
	public static final void enterWorld(AionConnection client, int objectId) {
		...
		if (player != null && client.setActivePlayer(player)) {
			...
			client.sendPacket(new SM_REQUEST_BEGINNER_SERVER(NetworkConfig.GAMESERVER_ID, FastTrackConfig.FASTTRACK_SERVER_ID, true));
			...
			// SM_INFLUENCE_RATIO, SM_SIEGE_LOCATION_INFO, SM_RIFT_ANNOUNCE
			// (Balaurea), SM_RIFT_ANNOUNCE (Tiamaranta)
			...
			if (CustomConfig.LOGIN_SERVICE_WELCOME_MESSAGE_ENABLED) {
				PacketSendUtility.sendYellowMessage(player, CustomConfig.LOGIN_SERVICE_WELCOME_MESSAGE);
			}
...
//				String bufferRegular = String.format(MembershipConfig.WELCOME_REGULAR,
//						new Object[] {GSConfig.SERVER_NAME, (int) (RateConfig.XP_RATE), (int) (RateConfig.QUEST_XP_RATE), (int) (RateConfig.DROP_RATE)});
//				String bufferVip = String.format(MembershipConfig.WELCOME_VIP,
//						new Object[] {GSConfig.SERVER_NAME, (int) (RateConfig.VIP_XP_RATE), (int) (RateConfig.VIP_QUEST_XP_RATE), (int) (RateConfig.VIP_DROP_RATE)});
//				String bufferPremium = String.format(MembershipConfig.WELCOME_PREMIUM,
//						new Object[] {GSConfig.SERVER_NAME, (int) (RateConfig.PREMIUM_XP_RATE), (int) (RateConfig.PREMIUM_QUEST_XP_RATE), (int) (RateConfig.PREMIUM_DROP_RATE)});
//				
...
//			}
			...
			if (player.isGM()) {
				if (AdminConfig.INVULNERABLE_GM_CONNECTION || AdminConfig.INVISIBLE_GM_CONNECTION || AdminConfig.ENEMITY_MODE_GM_CONNECTION.equalsIgnoreCase("Neutral")
					...
					if (AdminConfig.INVISIBLE_GM_CONNECTION) {
						player.getEffectController().setAbnormal(AbnormalState.HIDE.getId(), null);
					...
					if (AdminConfig.VISION_GM_CONNECTION) {
						player.setSeeState(CreatureSeeState.SEARCH20);
						PacketSendUtility.broadcastPacket(player, new SM_PLAYER_STATE(player), true);
			...
			// Homeward Bound Skill fix //FIXME: Remove this shit; it's ONLY learned from the book!
			...
			player.getController().addTask(TaskId.INVENTORY_UPDATE,
					ThreadPoolManager.getInstance().scheduleAtFixedRate(new ItemUpdateTask(player.getObjectId()), PeriodicSaveConfig.PLAYER_ITEMS * 1000, PeriodicSaveConfig.PLAYER_ITEMS * 1000));
