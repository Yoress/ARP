 ...
 * @rework Blackfire
 * @rework Kill3r
 * @modified Yon (Aion Reconstruction Project) -- {@link #addFusionStone(Item, int)} no longer wrongly attempts to update the database.
 */
public class ItemSocketService {
	...
	public static ManaStone addManaStone(Item item, int itemId) {
		...
		HashSet<Integer> allSlots = new HashSet<Integer>();
	...
	public static ManaStone addFusionStone(Item item, int itemId) {
		...
		/*
		 * This is out of place; it's inconsistent with the other methods.
		 * It also breaks storing the items in the database later as it
		 * pulls the top item stones instead of the bottom item stones.
		 * <p>
		 * This method will set the stone's PersistentState to UPDATED, even if it fails.
		 * So if it fails, the stone will be lost. Failure is expected if the item being
		 * socketed is in need of an update, or has manastones on the upper portion prior
		 * to the bottom portion. Just gonna comment the code and make it consistent.
		 */
