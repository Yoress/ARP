 ...
 * @author KID
 * @rework Blackfire
 * @modified Yon (Aion Reconstruction Project) -- changed {@link #addStackableItem(Player, ItemTemplate, long, ItemUpdatePredicate)}
 * to correctly stack power shards into the equipment slot.
 * @modified Alex.py (Aion Reconstruction Project) -- slight rework to allow for inventory overflow, especially from quests.
 */
public class ItemService {
	...
	public static long addItem(Player player, int itemId, long count, Item sourceItem, ItemUpdatePredicate predicate) {
		return addItem(player, itemId, count, sourceItem, predicate, false);
	}
	
	/**
	 * Add new item based on sourceItem values
	 */
	public static long addItem(Player player, int itemId, long count, Item sourceItem, ItemUpdatePredicate predicate, boolean allowOverflow) {
		...
		if (LoggingConfig.LOG_ITEM) {
			log.info("[ITEM] ID/Count" + (LoggingConfig.ENABLE_ADVANCED_LOGGING ? "/Item Name - " + itemTemplate.getTemplateId() + "/" + count + "/" + itemTemplate.getName()
		...
		if (itemTemplate.isStackable()) {
			count = addStackableItem(player, itemTemplate, count, predicate, allowOverflow);
		} else {
			count = addNonStackableItem(player, itemTemplate, count, sourceItem, predicate, allowOverflow);
		}
		
		if (!allowOverflow && inventory.isFull(itemTemplate.getExtraInventoryId()) && count > 0) {
	 ...
	 */
	private static long addNonStackableItem(Player player, ItemTemplate itemTemplate, long count, Item sourceItem, ItemUpdatePredicate predicate, boolean allowOverflow) {
		Storage inventory = player.getInventory();
		while ((allowOverflow || !inventory.isFull(itemTemplate.getExtraInventoryId())) && count > 0) {
	 ...
	 */
	private static long addStackableItem(Player player, ItemTemplate itemTemplate, long count, ItemUpdatePredicate predicate, boolean allowOverflow) {
		Storage inventory = player.getInventory();
	...
	public static boolean addQuestItems(Player player, List<QuestItems> questItems, ItemUpdatePredicate predicate) {
		...
		for (QuestItems qi : questItems) {
			addItem(player, qi.getItemId(), qi.getCount(), null, predicate, true);
		}
		return true;
	}
