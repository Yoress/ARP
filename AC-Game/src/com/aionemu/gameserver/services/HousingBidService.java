...
/**
 * @author Rolandas
 * @modified Yon (Aion Reconstruction Project) -- Prevented a couple of NullPointerException possibilities within {@link #executeTask()}.
 */
public class HousingBidService extends AbstractCronTask {
	...
	private int getBidsCountByType(Race race, HouseType type) {
		//INFO: This is pretty inefficient. Is it worth restructuring?
		int count = 0;
	...
	private void loadBidData() {
		...
		for (PlayerHouseBid playerBid : sortedBids) {
			...
			if (entry == null) {
				assert playerBid.getPlayerId() == 0; //The very first bid offer in terms of time should be placed by the system.
	...
	protected void executeTask() {
		...
		for (Entry<HouseBidEntry, Integer> sellData : successSell.entrySet()) {
			...
			//Bid price minus 10% commission, plus refund of the sale registration fee -- see in-game description.
			long returnKinah = ((long) (sellData.getKey().getBidPrice() * 0.9F)) + sellData.getKey().getRefundKinah();
			...
			if (LoggingConfig.LOG_HOUSE_AUCTION) {
				log.info("Address " + soldHouse.getAddress().getId() + " sold by player " + sellerPcd.getPlayerObjId() + " for price " + sellData.getKey().getBidPrice() + " (bid count: "
						+ sellData.getKey().getBidCount() + "; result: " + result + ") to player " + sellData.getKey().getLastBiddingPlayer());
			}
		}
		...
		for (Entry<HouseBidEntry, Integer> notSoldData : failedSell.entrySet()) {
			...
			if (bidHouse.isInGracePeriod()) {
				...
				if (timePassed > 7 * 24 * 3600) { // more than one week, i.e. 2 weeks passed
					...
					/*
					 * TODO: compensation here should be the amount the house was purchased for by the owner?
					 * Server takes house b/c it did not sell within the grace period; player can't own
					 * two houses, so this one is reclaimed.
					 */
				...
				if (bidHouse.getSellStarted() == null) {
					bidHouse.setSellStarted(new Timestamp(((long) getRunTime()) * 1000L));
				}
	...
	/**
	 * Remove house from auction. Currently from admin command, both for player and non-player houses. Returns kinah for the selling player and the
	 * last bidder if such exist
	 */
	public boolean removeHouseFromAuction(House house, boolean noSale) {
		...
		if (house.getOwnerId() != 0) {
			...
			MailFormatter.sendHouseAuctionMail(house, pcd, AuctionResult.CANCELED_BID, System.currentTimeMillis(), bidEntry.getRefundKinah());
			house.setStatus(HouseStatus.ACTIVE);
		} else {
		...
		if (lastPlayer != 0) {
			...
			if (pcd != null && playerBid != null) //playerBid shouldn't be null, but just in case.
				MailFormatter.sendHouseAuctionMail(house, pcd, AuctionResult.CANCELED_BID, System.currentTimeMillis(), playerBid.getBidPrice());
		}
		
		//Remove old bids for this house if they exist; needed for distinction between adding bid to database or changing an existing one.
		synchronized (playerBids) {
			for (Integer key: playerBids.keySet()) {
				HouseBidEntry bid = playerBids.get(key);
				if (bid != null && bid.getAddress() == house.getAddress().getId()) {
					playerBids.remove(key);
				}
			}
		}
	...
	public synchronized void placeBid(Player player, int entryIndex, long bidOffer) {
		...
		if (bidOffer > entry.getBidPrice() || entry.getBidCount() == 0) {
			...
			if (playerBids.put(player.getObjectId(), playerBid) == null) {
				DAOManager.getDAO(HouseBidsDAO.class).addBid(player.getObjectId(), bidHouse.getObjectId(), bidOffer, time);
			} else {
				DAOManager.getDAO(HouseBidsDAO.class).changeBid(player.getObjectId(), bidHouse.getObjectId(), bidOffer, time);
			}
