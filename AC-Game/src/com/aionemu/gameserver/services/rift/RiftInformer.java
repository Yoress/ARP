...
/**
 * @author Source
 * @modified Yon (Aion Reconstruction Project) -- Tried to prevent an exception.
 */
public class RiftInformer {
	...
	public static List<Npc> getSpawned(int worldId) {
		...
		//Might fix the exception?
		if (rifts instanceof ArrayList) rifts = (List<Npc>) ((ArrayList<Npc>) rifts).clone();
		List<Npc> worldRifts = new CopyOnWriteArrayList<Npc>();
		//FIXME: still throwing java.util.ConcurrentModificationException
		synchronized (rifts) {
			for (Npc rift : rifts) {
