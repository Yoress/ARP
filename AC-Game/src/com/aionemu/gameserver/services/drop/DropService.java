...
/**
 * @author ATracer, xTz
 * @modified Yon (Aion Reconstruction Project) -- Added a 30s decay time to Npc's without a decay task setup in
 * {@link #requestDropList(Player, int)}, {@link #closeDropList(Player, int)} execution order changed to clean
 * up the player state, deprecated {@link #see(Player, Npc)} as this is handled in SM_NPC_INFO.
 */
public class DropService {
	...
	public void requestDropList(Player player, int npcId) {
		...
		if (visObj instanceof Npc) {
			...
			if (decayTask != null) {
				...
				//30 Second decay time if there isn't a task set prior.
				//This mainly affects lootable quest Npc's that never have a decay time set.
				//This code may no longer be needed, but will be left here for redundancy.
				dropNpc.setReamingDecayTime(30 * 1000);
			}
		}
	...
	public void closeDropList(Player player, int npcId) {
		...
		//Don't return before cleaning up the player state!
		if (dropNpc == null) {
			return;
		}
			...
			for (Player member : dropNpc.getInRangePlayers()) {
				if (member != null && !requestedItem.getWinningPlayer().equals(member) && member.isOnline()) {
					PacketSendUtility.sendPacket(member,
							SM_SYSTEM_MESSAGE.STR_MSG_GET_ITEM_PARTYNOTICE(requestedItem.getWinningPlayer().getName(), new DescriptionId(ItemInfoService.getNameId(itemId))));
				}
			}
		}
	}
