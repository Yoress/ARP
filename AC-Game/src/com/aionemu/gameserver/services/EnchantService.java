...
public class EnchantService {
	...
	public static void amplifyItem(Player player, Item targetItem, Item material, Item tool) {
		...
		if (!targetItem.getItemTemplate().getExceedEnchant()) return;
		
		if (targetItem.getEnchantLevel() < targetItem.getMaxEnchantLevel()) return;
	...
	public static void amplifyItemCommand(Player player, Item item) {
		...
		if (!item.getItemTemplate().getExceedEnchant()) return;
		
		if (item.getEnchantLevel() < 15) return;
	...
	public static boolean enchantItem(Player player, Item parentItem, Item targetItem, Item supplementItem) {
		...
		// Since 4.7.5 we need to calculate the success for the new enchantment
		// stones a little bit different
		// Every new enchantment stone got a declared level range, so we pickup
		// random a level value for the
		// success calculation.
		...
		try {
			if (item.getItemTemplate().isWeapon()) {
				...
				if (CustomConfig.ENABLE_ENCHANT_BONUS) {
					...
					if (item.getEnchantLevel() > 0 && it != null && item.getEnchantLevel() < 22) { //TODO: Why is there a hardcoded cap?
						try {
				...
				// For the future...it looks like ncsoft will enable it also for
				// armors
			...
			if (!modifiers.isEmpty()) {
				player.getGameStats().addEffect(item, modifiers, false);
			}
		} catch (Exception ex) {
