...
public class RewardService {
	...
	public void verify(Player player) {
		...
		for (RewardEntryItem item : list) {
			...
			try {
				if (!SystemMailService.getInstance().sendMail("$$CASH_ITEM_MAIL", player.getName(), item.id + ", " + item.count, "0, " + (System.currentTimeMillis() / 1000) + ",", item.id,
						(int) item.count, 0, LetterType.BLACKCLOUD)) {
					continue;
				}
