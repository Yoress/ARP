...
/**
 * @author xTz
 * @modified Yon (Aion Reconstruction Project) -- modified release delay in {@link #release(Summon, UnsummonType, boolean)}
 * from 5000ms to 3400ms
 */
public class SummonsService {
	...
	public static final void release(final Summon summon, final UnsummonType unsummonType, final boolean isAttacked) {
		...
		//TODO: DISTANCE, LOGOUT, and UNSPECIFIED should release without a delay.
		summon.getObserveController().notifySummonReleaseObservers();
		summon.setReleaseTask(ThreadPoolManager.getInstance().schedule(new ReleaseSummonTask(summon, unsummonType, isAttacked), 3400));
	}
	
	/**
	 * @modified Yon (Aion Reconstruction Project) -- Added cooldown to master
	 */
	public static class ReleaseSummonTask implements Runnable {
		...
		public void run() {
			...
			SkillTemplate skill = DataManager.SKILL_DATA.getSkillTemplate(owner.getSummonSkillId());
			long cooldown = master.getSkillCooldown(skill);
			if (cooldown != 0) {
				cooldown = cooldown * 100 + System.currentTimeMillis();
				master.setSkillCoolDown(skill.getCooldownId(), cooldown);
				master.setSkillCoolDownBase(skill.getCooldownId(), System.currentTimeMillis());
			}
			...
			case UNSPECIFIED:
				...
				PacketSendUtility.sendPacket(master, new SM_SUMMON_PANEL_REMOVE(owner.getSummonSkillId()));
