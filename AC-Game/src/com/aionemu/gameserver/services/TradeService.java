...
public class TradeService {
	...
	public static boolean performBuyFromShop(Npc npc, Player player, TradeList tradeList) {
		...
		for (TradeItem tradeItem : tradeList.getTradeItems()) {
			...
			if (count != 0) {
				log.warn(String.format("CHECKPOINT: itemservice couldnt add all items on buy: %d %d %d %d", player.getObjectId(), tradeItem.getItemTemplate().getTemplateId(), tradeItem.getCount(),
						count));
	...
	public static boolean performBuyFromTradeInTrade(Player player, int npcObjectId, int itemId, int count, int TradeinListCount, int TradeinItemObjectId1, int TradeinItemObjectId2, int TradeinItemObjectId3) {
		 ...
		 * It has two means:
		 * 
		 * 1. ItemTemplates has not updated yet 2. Client Packet Hack
		 */
		...
		for (TradeinItem treadInList : itemTemplate.getTradeinList().getTradeinItem()) {
			...
			case 1:
				...
				if (item1.getItemId() != treadInList.getId()) AuditLogger.info(player, "Packet Hack. The Tradein items which sent by client are not same as Server.");
				return false;
			case 2:
