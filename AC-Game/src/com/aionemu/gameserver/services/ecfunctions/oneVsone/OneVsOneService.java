...
public class OneVsOneService implements OneVsOneStruct {
	...
	private void checkAnyPlayerReged() { // the main one that checks all the players and selects if there is
										 // any code for same code like player reg.. and ports.. DONE
		try {
	...
	public void startPort(final Player opponent1, final Player opponent2) {
		 //later make the ArenaMaster config to set here
		announceSpecificTwoOnly(opponent1, opponent2, ArenaMasterService.getInstance().ArenaMaster, "Found a Player to fight, You will be ported in 5sec...");
		opponent1.getEffectController().setAbnormal(AbnormalState.SLEEP.getId(), null);
		opponent1.getEffectController().updatePlayerEffectIcons();
		opponent1.getEffectController().broadCastEffects();
		
		opponent2.getEffectController().setAbnormal(AbnormalState.SLEEP.getId(), null);
