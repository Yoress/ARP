...
/**
 * @author SoulKeeper
 * @modified Yon (Aion Reconstruction Project) -- Added the ability to force capture an artifact.
 */
public class ArtifactSiege extends Siege<ArtifactLocation> {
	...
	protected void onSiegeFinish() {
		...
		// for artifact should be always true -- except when forced by balaur assault
		if (isBossKilled()) {
			onCapture();
		} else {
	...
	/**
	 * This method is used to force capture this artifact with the given {@link SiegeRace}.
	 */
	public void forceCapture(SiegeRace race) {
		getSiegeCounter().addRaceDamage(race, getBoss().getLifeStats().getMaxHp() + 1);
		
		//This prevents the warning that the boss wasn't slain.
		setBossKilled(true);
		
		//Just calling stop siege won't remove it from the list of active sieges on the siege service.
