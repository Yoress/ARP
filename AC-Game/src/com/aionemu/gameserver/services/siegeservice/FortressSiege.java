...
public class FortressSiege extends Siege<FortressLocation> {
	...
	public void onSiegeFinish() {
		...
		if (LoggingConfig.LOG_SIEGE) {
			if (winner != null) {
				log.info("[SIEGE] > Siege finished. [FORTRESS:" + getSiegeLocationId() + "] [OLD RACE: " + getSiegeLocation().getRace() + "] [OLD LegionId:" + getSiegeLocation().getLegionId()
						+ "] [NEW RACE: " + winner.getSiegeRace() + "] [NEW LegionId:" + (winner.getWinnerLegionId() == null ? 0 : winner.getWinnerLegionId()) + "]");
			} else {
...
//				SiegeAbyssRace ownerReward = getPlayerReward(player.getObjectId()); //TODO: This was unused, what was it for?
	...
	protected void giveRewardsToLegion() {
		...
		if (isBossKilled()) {
			if (LoggingConfig.LOG_SIEGE) {
				log.info("[SIEGE] > [FORTRESS:" + getSiegeLocationId() + "] [RACE: " + getSiegeLocation().getRace() + "] [LEGION :" + getSiegeLocation().getLegionId()
						+ "] Legion Reward not sending because fortress was captured(siege boss killed).");
			}
			return;
		}
		...
		if (legionBGeneral != 0) {
			...
			if (legionRewards != null) {
				for (SiegeLegionReward medalsType : legionRewards) {
					...
					MailFormatter.sendAbyssRewardMail(getSiegeLocation(), BGeneral, AbyssSiegeLevel.NONE, SiegeResult.PROTECT, System.currentTimeMillis(), medalsType.getItemId(),
	...
	protected void giveLossToPlayers(final SiegeRace race) {
		if (race == SiegeRace.BALAUR)// this shouldn't happen, but secure is
										// secure :)
			return;
		getSiegeLocation().doOnAllPlayers(new Visitor<Player>() {
			...
			@Override
			public void visit(Player player) {
				if (player.getRace().name() == race.name()) {// dont know if
																// this works...
