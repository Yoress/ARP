...
/**
 * @author Luzien
 * @modified Yon (Aion Reconstruction Project) -- Activated the siege region when spawning attackers (and checked to deactivate on assault finish).
 */
public class FortressAssault extends Assault<FortressSiege> {
	...
	public FortressAssault(FortressSiege siege) {
		...
		if (boss != null) {
			//Grab the siege region for later.
			region = boss.getActiveRegion();
		}
	}
		...
		dredgionTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				spawnTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
					...
					public void run() {
						spawnAttackers();
						if (region != null) region.siegeActivate();
					}
	...
	protected void onAssaultFinish(boolean captured) {
		...
		if (region != null) region.checkActiveness();
	...
	private void spawnRegularBalaurs() {
		...
		for (SpawnGroup2 spawnGroup : siegeSpawns) {
			for (SpawnTemplate spawnTemplate : spawnGroup.getSpawnTemplates()) {
				...
				SiegeSpawnTemplate spawn = SpawnEngine.addNewSiegeSpawn(spawnTemplate.getWorldId(), spawnTemplate.getNpcId(), locationId, SiegeRace.BALAUR, SiegeModType.ASSAULT, loc[0], loc[1],
