...
public class SiegeRaceCounter implements Comparable<SiegeRaceCounter> {
	...
	public Integer getWinnerLegionId() {
		...
		/*
		 * Map<Player, AtomicLong> teamDamageMap = new HashMap<Player, AtomicLong>(); for (Integer id : playerDamageCounter.keySet()) { Player player
		 * = World.getInstance().findPlayer(id);
		 * 
		 * if (player != null) { if (player.getCurrentTeam() != null) { Player teamLeader = player.getCurrentTeam().getLeaderObject(); long damage =
		 * playerDamageCounter.get(id).get(); if (teamLeader != null) { if (!teamDamageMap.containsKey(teamLeader)) { teamDamageMap.put(teamLeader,
		 * new AtomicLong()); } teamDamageMap.get(teamLeader).addAndGet(damage); } } } } if (teamDamageMap.isEmpty()) { return null; }
		 * 
		 * Player topTeamLeader = getOrderedCounterMap(teamDamageMap).keySet().iterator().next(); Legion legion = topTeamLeader.getLegion();
		 * 
