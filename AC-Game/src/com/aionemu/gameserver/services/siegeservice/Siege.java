 ...
 * @modified Yon (Aion Reconstruction Project) -- Fixed a bug caused by execution order in {@link #stopSiege()} for
 * artifact assaults.
 */
public abstract class Siege<SL extends SiegeLocation> {
	...
	public final void stopSiege() {
		if (finished.compareAndSet(false, true)) {
			if (this instanceof ArtifactSiege) {
				/*
				 * Because artifacts start a new siege (and assault) as the last task of ending their previous siege,
				 * the Balaur assault must be finished first, or the new one will be cancelled.
				 */
				if (SiegeConfig.BALAUR_AUTO_ASSAULT) {
