...
public class SourceSiege extends Siege<SourceLocation> {
	...
	protected void onSiegeFinish() {
		if (LoggingConfig.LOG_SIEGE) {
			//FIXME: Don't log a new race owning the source if the boss is alive.
			SiegeRaceCounter winner = getSiegeCounter().getWinnerRaceCounter();
			if (winner != null) {
				log.info("[SIEGE] > Siege finished. [SOURCE:" + getSiegeLocationId() + "] [OLD RACE: " + getSiegeLocation().getRace() + "] [OLD LegionId:" + getSiegeLocation().getLegionId()
