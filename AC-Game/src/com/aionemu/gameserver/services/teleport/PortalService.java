...
public class PortalService {
	...
	public static void port(final PortalPath portalPath, final Player player, int npcObjectId) {
		...
		case 0:
			...
			if (instance == null && group != null && !instanceGroupReq) {
				...
				for (Player member : group.getMembers()) {
					...
					// If some player is soloing and I found no one else yet, I
					// get his instance
					if (instance != null) {
						break;
					}
		...
		case 3:
			if (group != null || !instanceGroupReq) {
				// If there is a group (whatever group requirement exists or
				// not)...
				if (group != null) {
					instance = InstanceService.getRegisteredInstance(mapId, group.getTeamId());
				...
				if (instance == null && group != null && !instanceGroupReq) {
					...
					for (Player member : group.getMembers()) {
						...
						// If some player is soloing and I found no one else
						// yet, I get his instance
						if (instance != null) {
							break;
						}
					}
					...
					instance = registerGroup(group, mapId);
				} // No instance, default requirement off, no group = Register
					 // new instance with player ID
