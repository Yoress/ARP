...
public class BrokerService {
	...
	public void buyBrokerItem(Player player, int itemUniqueId, long itemCount) {
		...
		if (itemCount > buyingItem.getItemCount()) {
			...
			log.info("[BROKER EXCHANGE] > Malfunction data is received from packet [CM_BUY_BROKER_ITEM]. Buy items count are more than total item count." + " [Player: " + player.getName()
					+ "] bought [Item: " + buyingItem.getItemId() + "] " + "[Total Item Count: " + (buyingItem.getItemCount() + itemCount) + "] " + "[Buy Count: " + itemCount + "]"
					+ (LoggingConfig.ENABLE_ADVANCED_LOGGING ? " [Item Name: " + buyingItem.getItem().getItemName() : "]") + " from [Player: " + buyingItem.getSeller() + "] for [Price: "
					+ TotalBuyPrice + "]");
			return;
		}
		...
		synchronized (this) {
			...
			if (LoggingConfig.LOG_BROKER_EXCHANGE) {
				log.info("[BROKER EXCHANGE] > [Player: " + player.getName() + "] bought [Item: " + buyingItem.getItemId() + "] " + "[Total Item Count: " + (buyingItem.getItemCount() + itemCount)
						+ "] " + "[Buy Count: " + itemCount + "]" + (LoggingConfig.ENABLE_ADVANCED_LOGGING ? " [Item Name: " + buyingItem.getItem().getItemName() : "]") + " from [Player: "
	...
	public void CalcItemAveLowHigh(Player player, int itemUniqueId) {
		...
		CurrentLow = GetItemAveLowHigh(player, 1, itemUniqueId); // items's
																 // lowest
																 // price
		CurrentHigh = GetItemAveLowHigh(player, 2, itemUniqueId); // items's
																	 // highest
																	 // price
		Ave7day = GetItemAveLowHigh(player, 3, itemUniqueId); // 7-day item's
																 // price average
		IsLowHighSame = (CurrentLow == CurrentHigh ? true : false); // Calculate
																	 // "IsLowHighSame"
