...
/**
 * @author Mr. Poke
 * @modified Yon (Aion Reconstruction Project) -- {@link #getClosestCollision(float, float, float, float, float, float, boolean, boolean, int, byte)}
 * tweaked to be slightly less stupid when there aren't any collisions detected.
 */
public class GeoMap extends Node {
	...
	public String getDoorName(int worldId, String meshFile, float x, float y, float z) {
		...
		if (foundDoor == null) {
			// log.warn("Could not find static door: " + worldId + " " +
			// meshFile + " " + templatePoint);
			return null;
		}
	...
	public Vector3f getClosestCollision(float x, float y, float z, float targetX, float targetY, float targetZ, boolean changeDirection, boolean fly, int instanceId, byte intentions) {
		...
		if (results.size() == 0) {
			...
			//Remember that we added 1F to targetZ above? The code is pretty lenient, but close distances yo~
			if (Math.abs(geoZ - targetZ) < start.distance(end) || Math.abs(geoZ - (targetZ - 1F)) < start.distance(end)) {
