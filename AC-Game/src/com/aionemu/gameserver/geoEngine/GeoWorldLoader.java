...
/**
 * @author Mr. Poke
 * @modified Yon (Aion Reconstruction Project) -- {@link #loadMeshs(String)} modified to use unsigned short values for vectorCount and
 * to filter out meshes that have no collision intention. {@link #loadWorld(int, Map, GeoMap)} modified to no longer set the name
 * to lowercase when looking in the given String-Spatial map for the mesh. Ensured that RandomAccessFile objects are closed
 * in {@link #loadMeshs(String)}, and {@link #loadWorld(int, Map, GeoMap)}. Removed unused Logger.
 */
public class GeoWorldLoader {
	...
	public static Map<String, Spatial> loadMeshs(String fileName) throws IOException {
		...
		try {
			raFile = new RandomAccessFile(geoFile, "r");
			...
			while (geo.hasRemaining()) {
				...
				for (int c = 0; c < modelCount; c++) {
					...
					if (m.getIntentions() == 0) {
						//Non-filtered geodata, this mesh should not be loaded.
						m = null;
						continue;
					}
	...
	public static boolean loadWorld(int worldId, Map<String, Spatial> models, GeoMap map) throws IOException {
		...
		try {
			...
			while (geo.hasRemaining()) {
				...
				matrix3f.set(matrix);
				Spatial node = models.get(name/*.toLowerCase()*/.intern()); //Don't break the given geodata's system.
				try {
					if (node != null) {
	...
	/**
	 * Hash formula from paper http://www.beosil.com/download/CollisionDetectionHashing_VMV03.pdf Hash table size 50000, the higher value, more
	 * precision
	 */
