 ...
 * An eight sided box.
 * <p/>
 * A {@code Box} is defined by a minimal point and a maximal point. The eight vertices that make the box are then computed, they are computed in such
...
public abstract class AbstractBox extends Mesh {
	...
	protected final Vector3f[] computeVertices() {
		...
		return new Vector3f[] {	center.subtract(axes[0]).subtractLocal(axes[1]).subtractLocal(axes[2]), center.add(axes[0]).subtractLocal(axes[1]).subtractLocal(axes[2]),
								center.add(axes[0]).addLocal(axes[1]).subtractLocal(axes[2]), center.subtract(axes[0]).addLocal(axes[1]).subtractLocal(axes[2]),
								center.add(axes[0]).subtractLocal(axes[1]).addLocal(axes[2]), center.subtract(axes[0]).subtractLocal(axes[1]).addLocal(axes[2]),
								center.add(axes[0]).addLocal(axes[1]).addLocal(axes[2]), center.subtract(axes[0]).addLocal(axes[1]).addLocal(axes[2])};
	}
	 ...
	 * Rebuilds this box based on a new set of parameters.
	 * <p/>
	 * The box is updated so that the two opposite corners are {@code minPoint} and {@code maxPoint}, the other corners are created from those two
	 * positions.
	 *
