...
/**
 * <code>Node</code> defines an internal node of a scene graph. The internal node maintains a collection of children and handles merging said children
 * into a single bound to allow for very fast culling of multiple nodes. Node allows for any number of children to be attached.
 *
 * @author Mark Powell
 * @author Gregg Patton
 * @author Joshua Slack
...
public class Node extends Spatial implements Cloneable {
	...
	/**
	 * <code>attachChild</code> attaches a child to this node. This node becomes the child's parent. The current number of children maintained is
	 * returned. <br>
	...
	/**
	 * <code>attachChildAt</code> attaches a child to this node at an index. This node becomes the child's parent. The current number of children
	...
	/**
	 * <code>detachChild</code> removes a given child from the node's list. This child will no longe be maintained. Only the first child with a
	 ...
	 * between "^" and "$"). You can set regex modes, like case insensitivity, by using the (?X) or (?X:Y) constructs.
	 * </P>
	 * <p/>
	 ...
	 * <code>node.matches(aClass, aRegex)</code> + <code>node.descendantMatches(aClass, aRegex)</code>.
	 * <p/>
	 *
