...
/**
 * <code>Ray</code> defines a line segment which has an origin and a direction. That is, a point and an infinite ray is cast from this point. The ray
 * is defined by the following equation: R(t) = origin + t*direction for t >= 0.
 *
 * @author Mark Powell
 * @author Joshua Slack
 */
public final class Ray implements Cloneable, Collidable {
	...
	/**
	 * <code>intersectWhere</code> determines if the Ray intersects a triangle defined by the specified points and if so it stores the point of
	...
	/**
	 * <code>intersectWherePlanar</code> determines if the Ray intersects a triangle and if so it stores the point of intersection in the given loc
	 * vector as t, u, v where t is the distance from the origin to the point of intersection and u,v is the intersection point in terms of the
	 * triangle plane.
	 *
	 * @param t the Triangle to test against.
	...
	/**
	 * <code>intersectWherePlanar</code> determines if the Ray intersects a triangle defined by the specified points and if so it stores the point of
	 * intersection in the given loc vector as t, u, v where t is the distance from the origin to the point of intersection and u,v is the
	 * intersection point in terms of the triangle plane.
	 *
	 * @param v0 first point of the triangle.
	 * @param v1 second point of the triangle.
	 * @param v2 third point of the triangle.
	...
	/**
	 * <code>intersectWherePlanar</code> determines if the Ray intersects a quad defined by the specified points and if so it stores the point of
	 * intersection in the given loc vector as t, u, v where t is the distance from the origin to the point of intersection and u,v is the
	 * intersection point in terms of the quad plane. One edge of the quad is [v0,v1], another one [v0,v2]. The behaviour thus is like
	 * {@link #intersectWherePlanar(Vector3f, Vector3f, Vector3f, Vector3f)} except for the extended area, which is equivalent to the union of the
