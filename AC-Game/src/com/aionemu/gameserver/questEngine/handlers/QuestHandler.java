 ...
 * @modified vlog
 * @modified Yon (Aion Reconstruction Project) -- Added an ItemUseObserver to {@link #useQuestItem(QuestEnv, Item, int, int, boolean, int, int, int, int)},
 ...
 * {@link #sendQuestEndDialog(QuestEnv, int)} altered to close the dialog window upon quest completion,
 * {@link #defaultCloseDialog(QuestEnv, int, int, boolean, boolean, int, int, int, int, int, int)} altered to close the dialog window,
 * {@link #sendQuestEndDialog(QuestEnv, int)} altered to check for chained quests, added {@link #defaultCloseDialog(QuestEnv, int, int, boolean)},
 * added {@link #defaultCloseDialog(QuestEnv, int, int, boolean, int, int, int, int)}, removed unused and incorrect
 * method: {@link #sendItemCollectingStartDialog(QuestEnv)}, corrected misuse of {@link #sendQuestSelectionDialog(QuestEnv)} in
 * {@link #sendQuestStartDialog(QuestEnv, int, int, int)}.
 */
public abstract class QuestHandler extends AbstractQuestHandler implements ConstantSpawnHandler {
	...
	private void loadActionItems() {
		...
		for (QuestDrop drop : qDrop) {
			...
			if (actionItems == null) {
				actionItems = new HashSet<Integer>();
			}
	...
	public void changeQuestStep(QuestEnv env, int step, int nextStep, boolean reward, int varNum) {
		...
		if (qs != null && qs.getQuestVarById(varNum) == step) {
			//TODO: Test if nextStep should be ignored like this; example quest where it might break: Q11107
...
//					return sendQuestSelectionDialog(env);
//				}
				if (DataManager.QUEST_CONNECTIONS_DATA.hasConnectedQuest(questId)) {
					env.setQuestId(DataManager.QUEST_CONNECTIONS_DATA.getConnectedQuest(questId));
					if (QuestService.checkStartConditions(env, false) && QuestEngine.getInstance().isHaveHandler(env.getQuestId())) {
						env.setDialogId(31);
						if (QuestEngine.getInstance().onDialog(env)) {
							return true;
						}
						env.setDialogId(dialogId);
					}
	...
	public boolean defaultCloseDialog(QuestEnv env, int step, int nextStep, boolean reward) {
		return defaultCloseDialog(env, step, nextStep, reward, false);
	}
	...
	public boolean defaultCloseDialog(QuestEnv env, int step, int nextStep, boolean reward, int giveItemId, int giveItemCount, int removeItemId, int removeItemCount) {
		return defaultCloseDialog(env, step, nextStep, reward, false, 0, giveItemId, giveItemCount, removeItemId, removeItemCount);
	}
		...
		if (qs.getQuestVarById(varNum) == step) {
			...
			final ItemUseObserver observer = new ItemUseObserver() {
				...
				public void abort() {
					...
					PacketSendUtility.broadcastPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), item.getObjectId(), item.getItemId(), 0, 3, 0), true);
...
//			return sendQuestSelectionDialog(env); //FIX-ME: This should either close the dialog or send the correct dialog page
//		}
//		case QUEST_REFUSE_1: {
//			return sendQuestSelectionDialog(env); //FIX-ME: This should either close the dialog or send the correct dialog page
//		}
