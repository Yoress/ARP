 ...
 * @modified Yon (Aion Reconstruction Project) -- Updated handling for {@link PortalCooldownList}.
 */
public class SM_INSTANCE_INFO extends AionServerPacket {
	...
	public SM_INSTANCE_INFO(Player player, int instanceId) {
		...
		// this.maxcount =
		// DataManager.INSTANCE_COOLTIME_DATA.getInstanceCountByWorldId(instanceId)
		// != null ?
		// DataManager.INSTANCE_COOLTIME_DATA.getInstanceCountByWorldId(instanceId).getId()
		// : 0;
	}
			...
			if (hasTeam) { // all cooldowns from team
				...
				for (Player p : playerTeam.getMembers()) {
					...
					for (FastMap.Entry<Integer, PortalCooldown> e = cooldownList.getPortalCoolDowns().head(), end = cooldownList.getPortalCoolDowns().tail(); (e = e.getNext()) != end;) {
						...
						writeD(0x0);
						writeD((int) (e.getValue().getCurrentCooldown() - System.currentTimeMillis()) / 1000);
						// writeD(0x0); // unk 4.5
						// writeD(0x0); // unk 4.5
					}
					writeS(p.getName());
