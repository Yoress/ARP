 ...
 * @modified Yon (Aion Reconstruction Project) -- added {@link #getServerIPForClient(AionConnection)} to support chat server connections
 * for local clients.
 */
public class SM_VERSION_CHECK extends AionServerPacket {
	...
	protected void writeImpl(AionConnection con) {
		...
		// for... chat servers?
		{
			...
			writeB(getServerIPForClient(con));
			writeH(ChatService.getPort());
		}
	}
	
	/**
	 * Checks the given AionConnection's IP address and returns the proper server IP address for its range.
	 * <p>
	 * This method is called for the IP address the client should try connecting to for the chat server(?),
	 * but if the client is connected from a local IP address, the external IP of the server will not be a
	 * viable option to connect to; thus this method checks for such cases via {@link IPConfig} settings.
	 * <p>
	 * It's assumed that the chat server is running on the same machine as the game server.
	 * 
	 * @param con -- The AionConnection to get the server IP for.
	 * @return The IP address of this server for the given AionConnection.
	 */
	private byte[] getServerIPForClient(AionConnection con) {
		for (IPRange range: IPConfig.getRanges()) {
			if (range.isInRange(con.getIP())) {
				return range.getAddress();
			}
		}
		return IPConfig.getDefaultAddress();
	}
