 ...
 * @modified Yon (Aion Reconstruction Project) -- Removed some console spam, added ability for players to cancel item use with escape,
 * removed the incorrect system message stating you cannot use quest items. Removed the System Message packet that would inform
 * the player they used an item, as that should be elsewhere when the item's usage actually completes.
 */
public class CM_USE_ITEM extends AionClientPacket {
	...
	protected void runImpl() {
		...
		if (item == null) {
			//whole packet is 0 bytes when client is tuning an item. Probably sent so server can cancel other actions.
			if (uniqueItemId != 0 || type != 0) {
				log.warn(String.format("CHECKPOINT: null item use action: %d %d", player.getObjectId(), uniqueItemId));
			} else {
				player.getController().cancelUseItem();
				player.getController().onMove(); //causes an update for the client informing them of the cancellation.
			}
			return;
		}
		...
		if (itemActions == null) {
			if (!questItem) PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_ITEM_IS_NOT_USABLE);
			return;
		}
		...
		// Prevents potion spamming, and relogging to use kisks/aether
		// jelly/long CD items.
