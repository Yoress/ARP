 ...
 * @modified Yon (Aion Reconstruction Project) -- Log about possible timer cheat modified to no longer claim the player is being kicked when they are
 * not, added 98% tolerance for ping time interval.
 */
public class CM_PING extends AionClientPacket {
	...
	protected void runImpl() {
		...
		if (lastMS > 0 && player != null) {
			...
			//Adding 98% tolerance because this shit triggers without hacks on local clients
			if (pingInterval < (SecurityConfig.PING_INTERVAL * 1000)*0.98F) {// client timer cheat
				if (SecurityConfig.SECURITY_ENABLE) {
					...
					AuditLogger.info(player, "Possible client timer cheat. " + pingInterval + ", ip=" + getConnection().getIP());
				}
			}
		}
