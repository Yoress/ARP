...
public class CM_HOUSE_EDIT extends AionClientPacket {
	...
	protected void runImpl() {
		...
		} else if (action == HousingAction.EXIT_DECORATION) { // Exit Decoration
																 // mode
			sendPacket(new SM_HOUSE_EDIT(actionId));
		...
		} else if (action == HousingAction.ENTER_RENOVATION) { // enter
																 // renovation
																 // mode
			sendPacket(new SM_HOUSE_EDIT(14));
		} else if (action == HousingAction.EXIT_RENOVATION) { // exit renovation
																 // mode
			sendPacket(new SM_HOUSE_EDIT(15));
		} else if (action == HousingAction.CHANGE_APPEARANCE) { // update
																 // appearence
			House house = player.getHouseRegistry().getOwner();
