 ...
 * @modified Yon (Aion Reconstruction Project) -- Corrected colour value section.
 */
public class ManaStoneInfoBlobEntry extends ItemBlobEntry {
	...
	public void writeThisBlob(ByteBuffer buf) {
		...
		writeC(buf, item.getBonusEnchantLevel());// maxEnchant
		writeItemStones(buf);
...
//			writeD(buf, itemColor); //Replaced below.
			//itemColor variable is BGRA, where A is the least significant byte, R is the next least significant byte, etc...
			writeC(buf, (byte) ((itemColor & 0xFF00) >> 8)); //Red colour value
			writeC(buf, (byte) ((itemColor & 0xFF0000) >> 16)); //Green colour value
			writeC(buf, (byte) ((itemColor & 0xFF000000) >> 24)); //Blue colour value
			writeC(buf, (byte) itemColor); //Unknown value... using alpha
