...
/**
 * @author Rolandas
 * @reworked Yon (Aion Reconstruction Project) -- Added support for showing number of remaining tune attempts.
 */
public class PremiumOptionInfoBlobEntry extends ItemBlobEntry {
	...
	public void writeThisBlob(ByteBuffer buf) {
		...
		if (ownerItem.getRandomCount() >= ownerItem.getItemTemplate().getRandomBonusCount()) {
			writeC(buf, 1);
		} else {
			writeC(buf, 0);
		}
