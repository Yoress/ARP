 ...
 * @modified Yon (Aion Reconstruction Project) -- Added
 * {@link #estimateHeadingFrom(VisibleObject, VisibleObject)}, and {@link #diamondAngle(float, float)},
 * tweaked {@link #isInAttackRange(Creature, Creature, float)} to add onto offset for moving entities (instead of assigning it to 3).
 */
public class MathUtil {
	...
	/**
	 * Estimates the angle from the positive x-axis of the vector from the position of obj1 to obj2
	 * by calling {@link #diamondAngle(float, float)}, and then converts it to a heading value
	 * by multiplying it by 30F and casting as a byte.
	 * <p>
	 * The heading value returned is relative to the x-axis, and is in units of 3 degrees.
	 * What this means is that for every 3 degrees (counterclockwise) away from the x-axis
	 * the heading value returned will increase by 1.
	 * 
	 * @param obj1 -- The VisibleObject to use as a starting position for the vector.
	 * @param obj2 -- The VisibleObject to use as an ending position for the vector.
	 * @return The heading value needed for the first VisibleObject to face the other.
	 * @throws NullPointerException if either VisibleObject is null
	 */
	...
	/**
	 * Taken from https://www.freesteel.co.uk/wpblog/2009/06/05/encoding-2d-angles-without-trigonometry/
	 * <p>
	 * Outputs a value in the range [0, 4) describing the angle of a vector from the origin to the
	 * given Cartesian coordinate.
	 * 
	 * @param x - The x component of a coordinate to get the angle towards.
	 * @param y - The y component of a coordinate to get the angle towards.
	 * @return The angle of the vector from the origin to the given coordinates from standard position
	 * described as a value from [0, 4).
	 */
	...
	public final static boolean isInAttackRange(Creature object1, Creature object2, float range) {
		...
		//TODO: This is quite a distance to be adding here, maybe it should be toned down?
	...
	public final static Point get2DPointOnCircleCircumference(Point CenterPoint, Point EndPoint, int Radius) {
		// Calculates the Angle between the line through those two points
		// "StarPoint" and "EndPoint" and the X-Axis
	...
	public final static Point get2DPointInsideAnnulus(Point Center, int Radius1, int Radius2) {
		/*
		 * Circle 1 of Radius R1 Circle 2 of Radius R2
		 * 
		 * Where R1 > R2
		 * 
		 * Generate one Random Value for the Angular Value "theta", and one for the Distance from the Origin. As the Circles are both at the same
		 * origin this would be easy
		 */
	...
	/**
	 * Returns the correctly rounded square root of a positive BigDecimal. The algorithm for taking the square root of a BigDecimal is most critical
	 * for the speed of your application. This method performs the fast Square Root by Coupled Newton Iteration algorithm by Timm Ahrendt, from the
	 * book "Pi, unleashed" by Jörg Arndt in a neat loop.
	 *
