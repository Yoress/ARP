 ...
 * @author Imaginary
 * @rework Ever' for 4.5
 * @modified Yon (Aion Reconstruction Project) -- {@link #getRankForAp(int)} modified to not consider GP controlled ranks,
 * {@link #getRankForGp(int)} modified to not consider AP controlled ranks.
 */
@XmlEnum
public enum AbyssRankEnum {
	...
	public static AbyssRankEnum getRankForAp(int ap) {
		...
		for (AbyssRankEnum rank : values()) {
			if (rank.getId() > AbyssRankEnum.GRADE1_SOLDIER.getId()) continue;
	...
	public static AbyssRankEnum getRankForGp(int gp) {
		AbyssRankEnum rgp = AbyssRankEnum.GRADE9_SOLDIER;
		for (AbyssRankEnum rank : values()) {
			if (rank.getId() < AbyssRankEnum.STAR1_OFFICER.getId()) continue;
