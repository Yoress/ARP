...
public abstract class BattleGround {
	...
	public void teleportPlayer(Player player) {
		...
		if (player.getCommonData().getRace() == Race.ELYOS) {
			goTo(player, template.getWorldId(), template.getInsertPoint().getXe(), template.getInsertPoint().getYe(), template.getInsertPoint().getZe(), template.getInsertPoint().getHe());
		} else {
			goTo(player, template.getWorldId(), template.getInsertPoint().getXa(), template.getInsertPoint().getYa(), template.getInsertPoint().getZa(), template.getInsertPoint().getHa());
		}
	}
	...
	public void invitePlayer(final Player player) {
		PacketSendUtility.sendPacket(player,
				new SM_MESSAGE(0, null, "The Battleground: " + template.getName() + " is now ready to start. You will be teleported in 30 seconds. Have fun :)", ChatType.BRIGHT_YELLOW_CENTER));
