...
/**
 * This AI is intended for use by the small Fire Spirit mobs within Fire Temple. It may be moved into the open world AI list
 * in the future.
 * <p>
 ...
 */
@AIName("ftsmallfirespirit")
public class MercilessFireSpiritAI2 extends AggressiveNpcAI2 {
	
	int phase = 0;
	
	final static int FIRE_STRIKE = 16589,
					 STRIKE_HEAT = 16723;
	...
	protected void handleAttackComplete() {
		if (skillId == FIRE_STRIKE) phase = 1;
		if (skillId == STRIKE_HEAT) phase = 2;
	...
	public AttackIntention chooseAttackIntention(Creature creature) {
		...
		if (getOwner().getGameStats().canUseNextSkill()) {
			if (phase == 0 && getSkillList().isSkillPresent(FIRE_STRIKE)) {
				skillId = FIRE_STRIKE;
				skillLevel = getSkillList().getSkillLevel(FIRE_STRIKE);
				return AttackIntention.SKILL_ATTACK;
			}
			
			if (phase == 1 && getSkillList().isSkillPresent(STRIKE_HEAT)) {
				skillId = STRIKE_HEAT;
