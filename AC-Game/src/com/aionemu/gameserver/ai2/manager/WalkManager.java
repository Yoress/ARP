...
/**
 * @author ATracer
 * @modified Yon (Aion Reconstruction Project) -- {@link #findClosestRouteStep(Npc, List, RouteStep)} modified so it
 * can pick a point the NPC is already ontop of, {@link #chooseNextRandomPoint(NpcAI2)} modified to add Mechanics System support.
 */
public class WalkManager {
	...
	protected static RouteStep findClosestRouteStep(Npc owner, List<RouteStep> route, RouteStep nextStep) {
		...
		if (owner.getWalkerGroup() != null) {
			...
			for (RouteStep step : route) {
				...
				if (closestDist == -1 || stepDist < closestDist) {
	...
	private static void chooseNextRandomPoint(final NpcAI2 npcAI) {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				if (npcAI.isInState(AIState.WALKING)) {
					if (npcAI instanceof AbstractMechanicsAI2) {
						((AbstractMechanicsAI2) npcAI).onMechanicEvent(new GeneralMechanicEvent(MechanicEventType.on_stop_to_random_move));
					}
