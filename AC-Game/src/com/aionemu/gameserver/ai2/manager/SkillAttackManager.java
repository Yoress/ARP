...
import com.aionemu.gameserver.skillengine.condition.TargetFlyingCondition;
 ...
 * deprecated handling that's being moved inside AI's, tweaked {@link #chooseNextSkill(NpcAI2)} to support new skill list,
 * set skill cd for NpcSkillEntry in {@link #afterUseSkill(NpcAI2)} (removed from {@link #chooseNextSkill(NpcAI2)}).
 */
public class SkillAttackManager {
	...
	public static void afterUseSkill(NpcAI2 npcAI) {
		/*
		 * It's important that this doesn't crash, or the next attack won't be scheduled.
		 * AI's shouldn't use skills that aren't in their list, but server owners can
		 * make custom scripts do whatever. If owner is null, then the AI is rogue; so
		 * crashing is okay.
		 */
		NpcSkillList skillList = npcAI.getOwner().getSkillList();
		if (skillList != null) {
			NpcSkillEntry skill = skillList.getSkill(npcAI.getSkillId());
			if (skill != null) {
	...
	/**
	 * Returns a randomly selected skill from the given AI Owner's Skill List.
	 * 
	 * @param npcAI
	 * @return
	 */
	public static NpcSkillEntry chooseNextSkill(NpcAI2 npcAI) {
		...
		if (owner.getGameStats().canUseNextSkill()) {
			NpcSkillEntry npcSkill = skillList.getNextSkill();
			if (npcSkill != null/* && npcSkill.isReady()*/) { //Skill is known to be ready; it's checked in getNextSkill().
				...
				for (Condition condition: template.getStartconditions().getConditions()) {
					if (condition instanceof TargetFlyingCondition) {
						TargetFlyingCondition flyCondition = (TargetFlyingCondition) condition;
						try {
							if (!flyCondition.check((Creature) owner.getTarget())) {
								return null;
							}
							break;
