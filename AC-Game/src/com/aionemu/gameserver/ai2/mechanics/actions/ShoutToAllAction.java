 ...
 */
public class ShoutToAllAction extends Action {
	
	final public int stringId;
	
	public ShoutToAllAction(int stringId) {
		super(ActionType.shout_to_all);
	...
	public void performAction(MechanicEvent event, AbstractMechanicsAI2 ai) {
		...
		//TODO: Investigate what happens when this is called and adjust as needed.
		...
		owner.getKnownList().doOnAllPlayers(new Visitor<Player>() {
			...
			public void visit(Player player) {
				if (player.isOnline() && MathUtil.isIn3dRange(owner, player, range)) {
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(true, stringId, owner.getObjectId(), 2));
				}
			}
		});
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof ShoutToAllAction) {
			ShoutToAllAction o = (ShoutToAllAction) obj;
			return (o.stringId == stringId);
		}
		return false;
	}
