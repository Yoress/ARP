 ...
 * @modified Yon (Aion Reconstruction Project) -- ItemUseObserver in {@link #act(Player, Item, Item)}
 * now sends a cancel animation instead of a failure animation.
 */
	...
	public boolean canAct(Player player, Item parentItem, Item targetItem) {
		...
		if (CustomConfig.ENABLE_RIDE_RESTRICTION) {
			for (ZoneInstance zone : player.getPosition().getMapRegion().getZones(player)) {
				if (!zone.canRide()) {
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1401099)); //"You cannot get on the mount here."
					return false;
				}
			}
		}
