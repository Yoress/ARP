 ...
 * @modified Yon (Aion Reconstruction Project) -- {@link #act(Player, Item, Item, Item, int)} modified to use an
 * {@link ItemUseObserver} to watch for player cancellation, to no longer cancel chained manastones, and to remove remaining tune count.
 */
...
//		player.getController().cancelUseItem(); //Was preventing chained manastone socketing
		PacketSendUtility.broadcastPacketAndReceive(player,
				new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), parentItem.getObjectId(), parentItem.getItemTemplate().getTemplateId(), EnchantsConfig.ENCHANT_CAST_DELAY, 0, 0));
		...
		final ItemUseObserver observer = new ItemUseObserver() {
			...
			public void abort() {
				...
				PacketSendUtility.broadcastPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), parentItem.getObjectId(), parentItem.getItemId(), 0, 3, 0), true);
		...
		player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				//Remove tuning attempts
				targetItem.setRandomCount(targetItem.getItemTemplate().getRandomBonusCount());
				...
				PacketSendUtility.broadcastPacketAndReceive(player,
						new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), parentItem.getObjectId(), parentItem.getItemTemplate().getTemplateId(), 0, isSuccess ? 1 : 2, 0));
