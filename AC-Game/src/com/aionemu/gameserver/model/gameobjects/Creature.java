 ...
 *
 * @author -Nemesiss-
 * @modified Yon (Aion Reconstruction Project) -- isFlying and isInFlyingState methods renamed to isFlyingOrGliding and isFlying respectively for
 * clarity, added {@link #setType(CreatureType)}.
 */
public abstract class Creature extends VisibleObject {
	
	private static final Logger log = LoggerFactory.getLogger(Creature.class);
	protected AI2 ai2; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	private boolean isDespawnDelayed = false;
	private CreatureLifeStats<? extends Creature> lifeStats; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	private CreatureGameStats<? extends Creature> gameStats; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	private EffectController effectController; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	protected MoveController moveController; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	...
	private Skill castingSkill; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	...
	private final AggroList aggroList; //FIXME: Remove reference when Creature is removed from map; potential memory leak.
	...
	public boolean canSee(Creature creature) {
		...
		//TODO: Investigate! Hide 2 should work in most cases!
	 ...
	 */
	public boolean isFlyingOrGliding() {
		//TODO: Check if this is called by something applying stats for flying
	...
	/**
	 * Sets the {@link CreatureType} id of this Creature.
	 * <p>
	 * The default id is {@link CreatureType#NULL NULL}; should it be set with this method,
	 * subclasses should take it into consideration when returning the type of Creature for
	 * {@link com.aionemu.gameserver.network.aion.serverpackets.SM_NPC_INFO SM_NPC_INFO} via
	 * {@link #getType(Creature)}.
	 * <p>
	 * When using this method to change how an Npc is displayed on a temporary basis,
	 * the caller should also call this method again to set type back to NULL; this
	 * will allow the default handling to resume for the Creature.
	 * <p>
	 * For convenience, this method will immediately broadcast this creature's new state to
	 * all known players if it's an instance of {@link Npc}.
	 * <p>
	 * Subclasses cannot override this method.
	 * 
	 * @param type -- the CreatureType id to set for {@link #type}.
	 */
	public final void setType(CreatureType type) {
		boolean update = this.type != type.getId();
		this.type = type.getId();
		if (!update) return;
		final Creature thisCreature = this;
		if (thisCreature instanceof Npc)
			getKnownList().doOnAllPlayers(new Visitor<Player>() {
				@Override
				public void visit(Player player) {
					if (player.isOnline() & player.canSee(thisCreature)) {
						//FIXME: Replace with NPC Update packet when it's discovered; this packet will make the Npc blip out of existence for a sec
						PacketSendUtility.sendPacket(player, new SM_NPC_INFO((Npc) thisCreature, player));
//						PacketSendUtility.sendPacket(player, new SM_NPC_STATE(thisCreature));
					}
				}
			});
	}
