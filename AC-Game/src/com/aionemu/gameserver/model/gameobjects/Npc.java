 ...
 *
 * @author Luno
 * @modified Yon (Aion Reconstruction Project) -- removed {@link #isQuestBusy} and related methods ({@link #getIsQuestBusy()},
 * {@link #setIsQuestBusy(boolean)}), altered {@link #getType(Creature)} to consider overrides from super.
 */
public class Npc extends Creature {
	
	private WalkerGroup walkerGroup; //FIXME: Remove NPC from this group when the NPC is removed from the map; potential memory leak
//	@Deprecated
//	private boolean isQuestBusy = false; //TO-DO: Remove this junk
	...
	public Npc(int objId, NpcController controller, SpawnTemplate spawnTemplate, NpcTemplate objectTemplate, byte level) {
		...
		if (!aiOverride) {
			if (AIMechanics.isSupportedMechanic(objectTemplate.getMechanic())) {
				AI2Engine.getInstance().setupAI("mechanic_ai2", this);
			} else {
	...
	public int getType(Creature creature) {
		int typeForPlayer = super.getType(creature);
		if (typeForPlayer != CreatureType.NULL.getId()) {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				if (shout.getShoutType() == ShoutType.SAY || shout.getWhen() == ShoutEventType.IDLE) {
					...
					PacketSendUtility.sendPacket((Player) target, message); //FIXME: Doesn't send to other players in the area.
				} else {
					...
					while (iter.hasNext()) {
						...
						if (kObj.getLifeStats().isAlreadyDead() || !kObj.isOnline()) {
							continue; //FIXME: Dead players still receive messages, no?
						}
