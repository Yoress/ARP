...
/**
 * @author ATracer
 * @modified Yon (Aion Reconstruction Project) -- {@link #getAttackType()} now functions as whoever coded it intended.
 */
public class Homing extends SummonedObject<Creature> {
	...
	public ItemAttackType getAttackType() {
		if (getName().toLowerCase().contains("fire")) {
			return ItemAttackType.MAGICAL_FIRE;
		} else if (getName().toLowerCase().contains("stone")) {
			return ItemAttackType.MAGICAL_EARTH;
		} else if (getName().toLowerCase().contains("water")) {
			return ItemAttackType.MAGICAL_WATER;
		} else if ((getName().toLowerCase().contains("wind")) || (getName().toLowerCase().contains("cyclone"))) {
