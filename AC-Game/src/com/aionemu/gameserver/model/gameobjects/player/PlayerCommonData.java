 ...
 * @author Luno
 * @modified cura
 * @modified Yon (Aion Reconstruction Project) -- changed declaration of {@link #lastOnline} to be 20 seconds in the past as opposed to 20ms.
 */
public class PlayerCommonData extends VisibleObjectTemplate {
	...
	public void addExp(long value, RewardType rewardType, int npcNameId, String name) {
		...
		if (this.isReadyForReposteEnergy() && this.getCurrentReposteEnergy() > 0) {
			repose = (long) ((reward / 100f) * 40); // 40% bonus //FIXME: this isn't how this shit functions.
	...
	public boolean removeEventExp(long remove) {
		...
		if (eventExp < remove) return false;
	...
	public void setExp(long exp) {
		...
		if (getPlayerClass() != null && getPlayerClass().isStartingClass()) maxLevel = GSConfig.STARTING_LEVEL > GSConfig.STARTCLASS_MAXLEVEL ? GSConfig.STARTING_LEVEL : GSConfig.STARTCLASS_MAXLEVEL;
