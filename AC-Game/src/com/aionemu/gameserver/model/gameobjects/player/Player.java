 ...
 * @modified Yon (Aion Reconstruction Project) -- Added support for running on weapon draw.
 */
public class Player extends Creature {
	...
	private FriendList friendList; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private BlockList blockList;
	private PetList toyPetList; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private Mailbox mailbox; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private PrivateStore store; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private TitleList titleList; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private QuestStateList questStateList;
	private RecipeList recipeList;
	private List<House> houses;
	private ResponseRequester requester; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private boolean lookingForGroup = false;
	private boolean lookingForEvent = false;
	private Storage inventory;
	private Storage[] petBag = new Storage[StorageType.PET_BAG_MAX - StorageType.PET_BAG_MIN + 1]; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private Storage[] cabinets = new Storage[StorageType.HOUSE_WH_MAX - StorageType.HOUSE_WH_MIN + 1]; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	...
	private final AbsoluteStatOwner absStatsHolder; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private AbyssRank abyssRank;
	private NpcFactions npcFactions; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private Rates rates;
	...
	private FlyController flyController; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private CraftingTask craftingTask; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private int flightTeleportId;
	private int flightDistance;
	private Summon summon; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private SummonedObject<?> summonedObj; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private Pet toyPet; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private Kisk kisk; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	...
	private PortalCooldownList portalCooldownList; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	...
	/**
	 * Used to track if the Player was walking before setting {@link CreatureState#WEAPON_EQUIPPED}
	 * <p>
	 * Can be modified with {@link #setWalkingBeforeWeaponDraw(boolean)}, and checked with {@link #wasWalkingBeforeWeaponDraw()}.
	 */
	private boolean wasWalkingBeforeWeaponDraw = false;
	...
	private EmotionList emotions; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	private MotionList motions; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	...
	private SerialKiller skList; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	...
	// This variables are for the battleground system
	public BattleGround battleGround = null; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	...
	public BattleGroundFlag battlegroundFlag = null; //FIXME: Remove reference when Player should be GC'd (finalize method won't work here); potential memory leak.
	// 1 vs 1 Variables
	...
	public void setAttackMode(boolean isAttackMode) {
		if (isAttackMode) {
			if (isInState(CreatureState.WALKING)) {
				unsetState(CreatureState.WALKING);
				wasWalkingBeforeWeaponDraw = true;
			}
		} else {
			if (wasWalkingBeforeWeaponDraw) {
				setState(CreatureState.WALKING);
				wasWalkingBeforeWeaponDraw = false;
			}
		}
	...
	/**
	 * This method is called by {@link CM_MOVE} if the player sets walking while
	 * {@link CreatureState#WEAPON_EQUIPPED} is set.
	 * 
	 * @param walkingBeforeWeaponDraw
	 */
	public void setWalkingBeforeWeaponDraw(boolean walkingBeforeWeaponDraw) {
		wasWalkingBeforeWeaponDraw = walkingBeforeWeaponDraw;
	}
	
	/**
	 * @return {@link #wasWalkingBeforeWeaponDraw}.
	 */
	public boolean wasWalkingBeforeWeaponDraw() {
		return wasWalkingBeforeWeaponDraw;
	}
