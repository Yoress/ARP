 ...
 * @modified Yon (Aion Reconstruction Project) -- {@link #addAp(int)} changed to check if the current rank is due to GP before changing it,
 * {@link #addGp(int)} no longer attempts to change player rank (as that's controlled in AbyssRankUpdateService).
 */
public final class AbyssRank {
	...
	public void addAp(int additionalAp) {
		...
		//Also checks current rank -- It might be due to GP
		if (newRank.getId() <= 9 && rank.getId() <= 9) {
			setRank(newRank);
		}
	...
	public void addGp(int additionalGp) {
		...
		//Gaining or losing GP does NOT change your GP Rank. GP Ranks are controlled by AbyssRankUpdateService
...
//		}
	...
	public void setRank(AbyssRankEnum rank) {
		...
		//This is not how topRanking is used in the database, but it's an okay default.
		this.topRanking = rank.getQuota(); //TODO: Consider removing.
		setPersistentState(PersistentState.UPDATE_REQUIRED);
	}
	...
	public void doUpdate() {
		...
		// Checking the day - month & year are checked to prevent if a player
		// come back after 1 month, the same day
		...
		// Checking the week - year is checked to prevent if a player come back
		// after 1 year, the same week
