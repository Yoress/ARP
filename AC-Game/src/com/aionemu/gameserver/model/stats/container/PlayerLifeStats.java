 ...
 * @modified Yon (Aion Reconstruction Project) -- {@link #onIncreaseFp(TYPE, int, int, LOG)}
 * modified to not call {@link #sendAttackStatusPacketUpdate(TYPE, int, int, LOG)} if the
 * {@link TYPE} is {@link TYPE#NATURAL_FP}.
 */
public class PlayerLifeStats extends CreatureLifeStats<Player> {
	...
	public void updateCurrentStats() {
		...
		if (!owner.isFlyingOrGliding() && !owner.isInSprintMode()) {
			triggerFpRestore();
		}
	}
	...
	protected void onIncreaseFp(TYPE type, int value, int skillId, LOG log) {
		if (value > 0) {
			if (type != TYPE.NATURAL_FP) sendAttackStatusPacketUpdate(type, value, skillId, log);
