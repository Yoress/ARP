...
/**
 * @author xavier
 * @modified Yon (Aion Reconstruction Project) -- Removed all {@link Creature} casts of {@link #owner},
 * as type {@link T} already extends {@link Creature}. {@link #checkHPStats()} and {@link #checkMPStats()}
 * modified to only adjust HP and MP based on a percentage for players if they are not in combat or the
 * source of the effect was a skill (required refactor to {@link #checkHPStats(boolean)} and
 * {@link #checkMPStats(boolean)}); refactored {@link #onStatsChange()} to {@link #onStatsChange(boolean)},
 * refactored {@link #addEffect(StatOwner, List)} to {@link #addEffect(StatOwner, List, boolean)},
 * refactored {@link #endEffect(StatOwner)} to {@link #endEffect(StatOwner, boolean)}.
 */
public abstract class CreatureGameStats<T extends Creature> {
	...
	public final void addEffectOnly(StatOwner statOwner, List<? extends IStatFunction> functions) {
		...
		try {
			for (IStatFunction function : functions) {
				...
				// If already contains old stat, it won't add anyway, so next check will be false positive Useless Warning!!!
				/*TODO: Consider using.
	...
	public final void addEffect(StatOwner statOwner, List<? extends IStatFunction> functions, boolean fromSkill) {
		addEffectOnly(statOwner, functions);
		onStatsChange(fromSkill);
	}
	
	public final void endEffect(StatOwner statOwner, boolean fromSkill) {
		lock.writeLock().lock();
		try {
	...
	public boolean checkGeoNeedUpdate() {
		...
		if (currentTime - lastGeoUpdate > GeoDataConfig.GEO_VALID_TIME) {
	...
	protected void onStatsChange(boolean dueToSkill) {
		checkHPStats(dueToSkill);
		checkMPStats(dueToSkill);
	}
	
	private void checkHPStats(boolean dueToSkill) {
		...
		boolean canExploit = (!dueToSkill && (owner instanceof Player) && ((PlayerController) owner.getController()).isInCombat());
		boolean downwardsUpdate = oldHP.getCurrent() > newHP.getCurrent();
		if (oldHP.getCurrent() != newHP.getCurrent() && (!canExploit | downwardsUpdate)) {
	...
	private void checkMPStats(boolean dueToSkill) {
		...
		boolean canExploit = (!dueToSkill && (owner instanceof Player) && ((PlayerController) owner.getController()).isInCombat());
		boolean downwardsUpdate = oldMP.getCurrent() > newMP.getCurrent();
		if (oldMP.getCurrent() != newMP.getCurrent() && (!canExploit | downwardsUpdate)) {
