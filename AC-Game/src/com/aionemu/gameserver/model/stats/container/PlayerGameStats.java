...
/**
 * @author xavier
 * @modified Yon (Aion Reconstruction Project) -- Added call to {@link #onStatsChange()} in {@link #updateStatInfo()}.
 */
public class PlayerGameStats extends CreatureGameStats<Player> {
	...
	/*
	 * @Override public Stat2 getMAccuracy() { PlayerStatsTemplate pst = DataManager.PLAYER_STATS_DATA.getTemplate(owner.getPlayerClass(),
	 * owner.getLevel()); int base = pst.getMagicAccuracy(); Item mainHandWeapon = owner.getEquipment().getMainHandWeapon(); if (mainHandWeapon !=
	 * null) { base += mainHandWeapon.getItemTemplate().getWeaponStats().getMagicalAccuracy(); } return getStat(StatEnum.MAGICAL_ACCURACY, base); }
	 */
