...
public class PlayerTeamDistributionService {
	...
	public static void doReward(TemporaryPlayerTeam<?> team, float damagePercent, Npc owner, AionObject winner) {
		...
		int bonus = 100;
		if (size > 1) {
			bonus = 150 + (size - 2) * 10; //FIXME: Unintentional bonus exp
		}
		...
		for (Player member : filteredStats.players) {
			...
			if (filteredStats.highestLevel - member.getLevel() >= 10) {
				...
				int cape = XPCape.values()[(int) member.getLevel()].value(); //FIXME: This can crash with players above 65
				if (cape < rewardXp) {
					rewardXp = cape;
				}
			}
			...
			if (member.isMarried() && member.getPlayerGroup2().getMembers() == partner && member.getPlayerGroup2().getMembers().size() == 2) { //FIXME: This will always fail?
