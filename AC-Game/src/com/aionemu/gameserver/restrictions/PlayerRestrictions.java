 ...
 * @Reworked Kill3r
 * @modified Yon (Aion Reconstruction Project) -- Removed flying restriction on flight time restoration.
 */
public class PlayerRestrictions extends AbstractRestrictions {
	...
	public boolean canAffectBySkill(Player player, VisibleObject target, Skill skill) {
		...
		//TODO: What's the purpose of the floating corpse check?
		...
		//Removed because there are no longer any restrictions for this.
		//Code left behind as a courtesy.
//		if (skill.getSkillTemplate().hasItemHealFpEffect() && !player.isFlying()) { // player must be
...
//		}
		...
		// if (!skill.getSkillTemplate().hasHealEffect() ||
		// !skill.getSkillTemplate().hasRandomMoveEffect()
		// || skill.getSkillTemplate().getSubType() != SkillSubType.BUFF ||
		// skill.getSkillTemplate().getSubType() != SkillSubType.CHANT
		// || skill.getSkillTemplate().getSubType() != SkillSubType.SUMMON ||
		// skill.getSkillTemplate().getSubType() != SkillSubType.SUMMONHOMING) {
		// if (target instanceof Player && ((Player)
		// target).isInsideZoneType(ZoneType.PVP)) {
		// List<ZoneInstance> zones =
		// target.getPosition().getMapRegion().getZones((Player) target);
