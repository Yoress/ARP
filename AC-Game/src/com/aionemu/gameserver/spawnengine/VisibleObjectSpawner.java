...
/**
 * @author ATracer
 * @modified Yon (Aion Reconstruction Project) -- modified {@link #spawnSummon(Player, int, int, int, int)} to spawn
 * summons in front of the player instead of ontop of them, modified {@link #spawnPet(Player, int)} to spawn pets
 * next to the player instead of ontop of them, modified {@link #spawnTrap(SpawnTemplate, int, Creature, int)}
 * to spawn traps in the appropriate hide effect and also tweaked the exception handling.
 */
public class VisibleObjectSpawner {
	...
	public static Trap spawnTrap(SpawnTemplate spawn, int instanceIndex, Creature creator, int skillId) {
		...
		try {
			//ai.TrapNpcAI2.EVENT_SET_TRAP_RANGE == 1
			trap.getAi2().onCustomEvent(1, DataManager.SKILL_DATA.getSkillTemplate(skillId).getProperties().getEffectiveRange());
		} catch (Exception e) {
			/*
			 * Who put this here for exception handling? DeliveryManAI2 is the only AI that can use
			 * this type of event (passing in a Creature) at the time of writing; that AI should not
			 * be on any Traps.
			 */
//			trap.getAi2().onCustomEvent(1, creator);
			log.error("[VisibleObjectSpawner#spawnTrap()]: Unable to set range on trap: " + objectId, e);
		}
	...
	public static Npc spawnPostman(final Player owner) {
		...
		Vector3f pos = GeoService.getInstance().getClosestCollision(owner, owner.getX() + (float) (Math.cos(radian) * 5), owner.getY() + (float) (Math.sin(radian) * 5), owner.getZ(), false,
				CollisionIntention.PHYSICAL.getId());
	...
	public static Npc spawnFunctionalNpc(final Player owner, int npcId, SummonOwner summonOwner) {
		...
		int instanceId = owner.getInstanceId();
		double radian = Math.toRadians(MathUtil.convertHeadingToDegree(owner.getHeading()));
		Vector3f pos = GeoService.getInstance().getClosestCollision(owner, owner.getX() + (float) (Math.cos(radian) * 1), owner.getY() + (float) (Math.sin(radian) * 1), owner.getZ(), false,
				CollisionIntention.PHYSICAL.getId());
		SpawnTemplate spawn = SpawnEngine.addNewSingleTimeSpawn(worldId, npcId, pos.getX(), pos.getY(), pos.getZ(), (byte) 0);
	...
	public static Summon spawnSummon(Player creator, int npcId, int skillId, int skillLevel, int time) {
		...
		float x = creator.getX() + 2*FastMath.cos(FastMath.DEG_TO_RAD * (heading*3));
		float y = creator.getY() + 2*FastMath.sin(FastMath.DEG_TO_RAD * (heading*3));
		float z = creator.getZ();
		if (GeoDataConfig.GEO_ENABLE) {
			//Prevent spawning in walls
			if (!GeoService.getInstance().canSee(creator.getWorldId(), creator.getX(), creator.getY(), creator.getZ(), x, y, z, 3, creator.getInstanceId())) {
				x = creator.getX();
				y = creator.getY();
			}
		}
//		heading = (byte) ((((int) heading) + 60) % 120); //Heading updates to match player almost instantly for summons, so no point
		...
		Summon summon = new Summon(IDFactory.getInstance().nextId(), isSiegeWeapon ? new SiegeWeaponController(npcId) : new SummonController(), spawn, npcTemplate,
		...
		summon.setSummonSkillId(skillId);
...
//		byte heading = player.getHeading();
		...
		byte heading = player.getHeading();
		float x = player.getX() + 0.9F*FastMath.cos(FastMath.DEG_TO_RAD * (((heading + 30) % 120)*3));
		float y = player.getY() + 0.9F*FastMath.sin(FastMath.DEG_TO_RAD * (((heading + 30) % 120)*3));
		float z = player.getZ();
		if (GeoDataConfig.GEO_ENABLE) {
			//Prevent spawning in walls
			if (!GeoService.getInstance().canSee(player.getWorldId(), player.getX(), player.getY(), player.getZ(), x, y, z, 3, player.getInstanceId())) {
				x = player.getX();
				y = player.getY();
			}
		}
//		heading = (byte) ((((int) heading) + 60) % 120);
