...
public class SpawnEngine {
	...
	public static VisibleObject spawnObject(SpawnTemplate spawn, int instanceIndex) {
		...
		spawn.addVisibleObject(visObj, instanceIndex);
		return visObj;
	}
	...
	/**
	 * Should be used when need to define whether spawn will be deleted after death Using this method spawns will not be saved with //save_spawn
	 * command
	 *
	 * @param worldId
	 * @param npcId
	...
	public static void spawnInstance(int worldId, int instanceId, byte difficultId, int ownerId) {
		...
		if (worldSpawns != null) {
			for (SpawnGroup2 spawn : worldSpawns) {
				...
				// Disable temporary spawns in instances, TemporarySpawnEngine doesn't support removing spawns
