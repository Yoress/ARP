...
public class InstanceWalkerFormations {
	...
	public InstanceWalkerFormations() {
		...
		formationVariants = new HashMap<String, List<WalkerGroup>>();
		walkerVariants = new HashMap<String, List<ClusteredNpc>>();
	}
			...
			if (maxSize == 1) {
				if (candidates.size() != 1) {
					...
					if (singleNpc.getWalkTemplate().getVersionId() != null) {
						...
						if (variants == null) {
							variants = new ArrayList<ClusteredNpc>();
				...
				if (wg.getVersionId() == null) {
					...
					if (variants == null) {
						variants = new ArrayList<WalkerGroup>();
