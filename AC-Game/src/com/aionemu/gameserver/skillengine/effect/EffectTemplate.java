...
/**
 * @author ATracer
 * @modified Yon (Aion Reconstruction Project) -- commented out pull resistance for entities that cannot move in
 * {@link #isImuneToAbnormal(Effect, StatEnum)}.
 */
...
public abstract class EffectTemplate {
	 ...
	 * check cannotmiss 7) check magic resist / dodge 8) addsuccess
	 * <p/>
	 * exceptions: buffbind buffsilence buffsleep buffstun randommoveloc recallinstant returneffect returnpoint shieldeffect signeteffect summoneffect
	 * xpboosteffect
	 *
	 * @param effect
	 * @param statEnum
			...
			if (effected instanceof Npc) {
				...
				if (npc.getObjectTemplate().getStatsTemplate().getRunSpeed() == 0) {
					if (/*statEnum == StatEnum.PULLED_RESISTANCE ||*/ statEnum == StatEnum.STAGGER_RESISTANCE || statEnum == StatEnum.STUMBLE_RESISTANCE) {
						//PULLED_RESISTANCE does not belong here; entities that cannot move can still be pulled!
						return true;
					}
				}
			}
	...
	/**
	 * certain effects are magical even when used in physical skills it includes stuns from chanter/sin/ranger etc these effects(effecttemplates) are
	...
	private boolean isMagicalEffectTemp() {
		if (this instanceof SilenceEffect || this instanceof SleepEffect || this instanceof RootEffect || this instanceof SnareEffect || this instanceof StunEffect || this instanceof PoisonEffect
				|| this instanceof BindEffect || this instanceof BleedEffect || this instanceof BlindEffect || this instanceof DeboostHealEffect || this instanceof ParalyzeEffect
