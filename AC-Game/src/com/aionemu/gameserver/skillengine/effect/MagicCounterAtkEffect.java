					...
					public void run() {
						if (skill.getSkillTemplate().getType() == SkillType.MAGICAL && skill.getSkillTemplate().getSubType() == SkillSubType.ATTACK) {
							if ((int) (cls.getMaxHp() / 100f * value) <= maxdmg) {
								effected.getController().onAttack(effector, effect.getSkillId(), TYPE.DAMAGE, (int) (cls.getMaxHp() / 100f * value), true, LOG.REGULAR, null);
							} else {
								//TODO: Why is this different here?
								effected.getController().onAttack(effector, maxdmg, true, null);
							}
						}
					}
				}, 0);
