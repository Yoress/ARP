...
CREATE TABLE `houses` (
  ...
  `acquire_time` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  ...
  PRIMARY KEY (`id`),
#  UNIQUE KEY `address_2` (`address`), # Why is this here? Studio addresses are shared, they cannot be unique
  KEY `address` (`address`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `inventory` (
  ...
  `max_enchant` tinyint(3) DEFAULT '0',
